...
Implemented in this prompt. The next safe runtime step is to apply migration 264 in a controlled environment, run reconciliation against real JSON/PG stores, and block POSTGRES_ONLY while fallback/drift counters are non-zero.
...
- Migration reviewer: migration 264 is additive and guarded with `IF NOT EXISTS`; live apply is still required.
