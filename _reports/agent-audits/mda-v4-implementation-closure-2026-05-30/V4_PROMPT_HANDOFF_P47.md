...
P47 adds `RuntimeRequirementResolverService`, `GateContextBuilder`, `RuntimeRequirementGateException`, and migration `273_runtime_requirement_resolver.sql`. The resolver is fail-closed by default and rejects caller `require_*` truth. The gate is wired into mobile and dispatch StartJob paths before mutation.
