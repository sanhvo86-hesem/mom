...
## Multi-AI Concurrent Work Protocol

**Violations here are the primary cause of data loss in this repository. Every AI agent must read this before touching any file.**

### One branch — one AI session — always

- Each Claude window, GPT session, Codex task, or Copilot edit = exactly one `codex/<task-slug>` branch, created fresh from `main`.
- **Never share a branch between two AI windows simultaneously.** The second writer overwrites or conflicts with the first.
- To hand off work from one AI session to another: commit + push on the source session first, then open the target session on that branch.
- Before starting work, always run `git branch -r` and check what remote branches exist and what other agents may be active on.

### Commit and push cadence — non-negotiable

| When | Required action |
|---|---|
| After every logical unit of work | `git add <specific-files> && git commit -m "..."` |
| Before closing a window or switching context | `git push origin <branch>` |
| Before any `git checkout`, `git merge`, or `git rebase` | Commit and push first — no exceptions |
| Before opening a new AI session on the same repo | Push from the previous session so the VPS data-sync hook does not conflict |

Uncommitted work is invisible to all other agents and to the VPS sync. Unpushed commits are lost on session reset. The startup data-sync hook pulls VPS runtime config at session open — uncommitted local edits to tracked files are silently replaced.

### Data loss scenarios specific to this repo

1. **Migration number collision.** Two AI sessions each create `NNN_xxx.sql` with the same number. The second push wins; the first breaks the migration runner. Before naming any migration: `git log --all --oneline -- mom/database/migrations/ | tail -5` to confirm the highest existing number.

2. **VPS config overwrite on session open.** The startup hook pulls `users.json`, `module_access_config.json`, `deploy/*.json` from the VPS. Uncommitted local edits to these files are silently replaced. Commit before closing any session that touched config files.

3. **Tracked HTML doc discarded on branch switch.** Files under `mom/docs/operations/**/*.html` are git-tracked. Uncommitted edit + `git checkout` = conflict block or silent restore. Either way the work is gone. Commit first.

4. **Force push orphans another AI's commits.** If AI #1 rebases and force-pushes while AI #2 has local commits on the same branch, AI #2's commits are orphaned. Never `git push --force` on any shared or active branch.

5. **Stale AI index read by a parallel session.** `.ai/symbols.json`, `.ai/db-map/*.json` may be regenerated by one AI while another reads stale copies. Regenerate after significant changes: `php tools/scripts/ai-index/generate.php --verbose`.

### Cherry-pick to main — the ONLY integration path

**Never merge a codex/* branch directly to main while other AI sessions are active.**
Direct merge pulls in the whole branch history and overwrites files the other session also
modified. Cherry-pick applies only the commits that belong to this session.

**Mandatory integration sequence when done:**

```
# 1. Ensure all commits are pushed to your codex/* branch
git push origin <your-branch>

# 2. Run the cherry-pick staging script (creates cherry/YYYYMMDD-<slug> from origin/main)
bash tools/ai/cherry-pick-to-main.sh

# 3. Push the staging branch and open a PR (or merge directly if solo)
git push origin cherry/<YYYYMMDD-slug>
gh pr create --base main --head cherry/<YYYYMMDD-slug>
```

The script:
- Detects file overlap with other active `codex/*` sessions and warns before proceeding
- Cherry-picks your commits oldest→newest onto a clean fork of `origin/main`
- Runs PHP syntax check + migration drift check
- Leaves your source branch untouched

**If the script reports a cross-branch collision:**
1. Coordinate — one session pushes first
2. The other session rebases its staging branch onto the updated `origin/main`
3. Only then does the second session push

**`--push` flag** pushes the staging branch immediately; omit it to review first.

### Branch deletion gate — ALL 4 must pass

Do NOT delete a branch until every gate below is verified:

| Gate | Verification command | Expected result |
|---|---|---|
| 1. PR merged (Merged, not just Closed) | `gh pr view <number> --json state -q .state` | `MERGED` |
| 2. CI green (deploy.yml succeeded on main) | `gh run list --branch main --limit 3` | workflow status `completed` |
| 3. VPS deployed (merge commit on server) | `ssh eqms 'git -C /var/www/eqms.hesemeng.com log --oneline -1'` | merge commit SHA matches |
| 4. Browser smoke test passed | Open eqms.hesemeng.com — confirm the changed feature works end-to-end | no regression |

Only after all 4 pass: `git push origin --delete codex/<branch-name>`

**Unmerged-commit check (run before deletion):** `git log main..codex/<branch> --oneline` — must return empty.

**Never delete:** `main`, any branch where the unmerged-commit check is non-empty, any branch referenced by an open PR, or any branch not created by an AI agent (ask the user first).
...
- After a successful merge, verify all 4 branch deletion gates above pass before deleting the branch.
