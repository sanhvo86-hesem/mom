...
class AuthController extends BaseController
{
    ...
    public function status(): never
    {
        ...
        if ($preauthUser !== '' && $preauthRemaining <= 0) {
            clear_pending_auth_session_state();
            $authExpired = $authExpired ?: 'mfa';
            $preauthUser = '';
        }

        if ($preauthUser !== '' && is_array($this->store)) {
            /** @var array<string, mixed>|false $u */
    ...
    public function login(): never
    {
        ...
        if (!$user || !($user['active'] ?? true)) {
            ...
            // Use a valid bcrypt hash at cost=12 to match real password hashing.
            password_verify($password, '$2y$12$R9h7cIPz0gi.URNNX3kh2OPST9/PgBkqquzi.Ss7KIUgO2qt0KLOC');
            usleep(150000);
