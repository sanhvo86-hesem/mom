...
/**
 * MDA-side bridge into the existing UOM runtime authority.
 *
 * This class deliberately does not implement conversion rules, rounding, alias
 * policy, or item UOM policy. It only resolves command quantities through the
 * UOM services so MDA command handlers can depend on one authority surface
 * without editing the parallel UOM implementation branch.
 */
final class MdaUomAuthorityBridge
{
    public const AUTHORITY = 'existing_uom_runtime_authority';
    ...
    /**
     * Normalize a governed command quantity through the existing UOM authority.
     *
     * The method is intentionally fail-closed: missing item, unit, magnitude, or
     * UOM policy produces a UomException before callers mutate domain truth.
     *
    ...
    public function normalizeCommandQuantity(string $commandName, array $payload, array $context = []): array
    {
        ...
        if ($policy === null) {
            throw new UomException(
                ...
                "Command '{$commandName}' has no MDA UOM authority policy.",
                409
            );
        }
