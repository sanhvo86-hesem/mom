...
BASH;
    ...
    private function isAllowedCommand(string $command): bool
    {
        ...
        if ($this->isAllowedControlEndpointProbeCommand($command)) {
            return true;
        }
        return $this->isAllowedFileExplorerCommand($command);
    }
    ...
    /**
     * SVC-011: Validate file-explorer commands generated by fileExplorerCommand().
     *
     * The file-explorer command is a PHP heredoc inline script preceded by a
     * known set of VPS_EXPLORER_* environment-variable assignments built with
     * escapeshellarg().  Validation strategy:
     *
     *   1. Split at the fixed heredoc delimiter.  The body after the delimiter
     *      must match fileExplorerPhpScript() exactly — no injected PHP.
     *   2. The prefix must contain only VPS_EXPLORER_* variable assignments.
     *      No other variable name is allowed — prevents env-injection pivots.
     *   3. VPS_EXPLORER_OP must be present (sanity check).
     *
     * Values are always single-quoted by escapeshellarg() so shell-injection
     * through a value is not possible regardless of value content.
     */
    private function isAllowedFileExplorerCommand(string $command): bool
    {
        $delimiter = " php <<'__VPS_FILE_EXPLORER_PHP__'\n";
        $pos = strpos($command, $delimiter);
        if ($pos === false) {
            return false;
        }

        // Body must be the exact known PHP script — no tampering allowed.
        $body = substr($command, $pos + strlen($delimiter));
        if ($body !== $this->fileExplorerPhpScript() . "\n__VPS_FILE_EXPLORER_PHP__") {
            return false;
        }

        // Env prefix: only VPS_EXPLORER_* variable names are permitted.
        $envPart = substr($command, 0, $pos);
        $allowedKeys = [
            'VPS_EXPLORER_OP',
            'VPS_EXPLORER_ROOT',
            'VPS_EXPLORER_PATH',
            'VPS_EXPLORER_TARGET_PATH',
            'VPS_EXPLORER_NAME',
            'VPS_EXPLORER_UPLOAD_TMP',
            'VPS_EXPLORER_QUERY',
            'VPS_EXPLORER_HIDDEN',
            'VPS_EXPLORER_LIMIT',
            'VPS_EXPLORER_MAX_BYTES',
            'VPS_EXPLORER_DOWNLOAD',
            'VPS_EXPLORER_OVERWRITE',
            'VPS_EXPLORER_READ_ONLY',
            'VPS_EXPLORER_DENY',
        ];

        // Extract every shell-variable name from the env prefix.
        preg_match_all('/(?:^| )([A-Za-z_][A-Za-z0-9_]*)=/', $envPart, $nameMatches);
        if ($nameMatches[1] === []) {
            return false;
        }
        foreach ($nameMatches[1] as $varName) {
            if (!in_array($varName, $allowedKeys, true)) {
                return false;
            }
        }

        // VPS_EXPLORER_OP must be present so we know an operation was specified.
        return in_array('VPS_EXPLORER_OP', $nameMatches[1], true);
    }
