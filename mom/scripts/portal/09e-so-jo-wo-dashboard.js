...
/* PR #5 — Source attribution helpers.
 * AEOI commits stamp source='ai_order_intake' + source_record_id on the
 * sales_order record; we surface it as a clickable pill that takes the
 * operator back to the originating intake case. Non-AEOI orders get no
 * pill at all so the dashboard stays uncluttered for human-entered work. */
function _isAiSource(item){ return item && (item.source === 'ai_order_intake' || item.source_system === 'AEOI'); }
function _aiSourcePill(item, opts){ if(!_isAiSource(item)) return ''; var ref=item.source_record_id||''; var size=(opts&&opts.size)||'sm'; var pad=size==='lg'?'4px 10px':'2px 7px'; var fs=size==='lg'?'12px':'10px'; var title=_t('Đến từ AEOI Intake','From AEOI Intake')+(ref?' — '+ref:''); return '<span class="sj-ai-pill" data-ai-source="'+_esc(ref)+'" style="display:inline-flex;align-items:center;gap:4px;padding:'+pad+';border-radius:10px;font-size:'+fs+';font-weight:600;background:#dbeafe;color:#1e40af;cursor:pointer" title="'+_esc(title)+'">🤖 AI'+(size==='lg'&&ref?' · <span style="font-family:ui-monospace,monospace;font-weight:500">'+_esc(ref)+'</span>':'')+'</span>'; }
function _renderRecordBadges(item){ var type=_recordType(item), band=_bandMeta(_recordBand(item)), phase=_phaseMeta(_recordPhase(item)), readiness=_recordReadiness(item), st=_status(type,item.status||''); var h='<div class="sj-record-badges"><span class="sj-type-pill type-'+type+'">'+type.toUpperCase()+'</span>'+_aiSourcePill(item)+'<span class="sj-band-pill" style="background:'+band.bg+';color:'+band.color+'">'+_esc(_t(band.vi,band.en))+'</span><span class="sj-phase-pill">'+_esc(_t(phase.vi,phase.en))+'</span><span class="sj-status" style="color:'+st.color+';background:'+_rgba(st.color,.10)+'">'+_esc(st.text)+'</span>'; if(readiness!=null) h+='<span class="sj-score-pill">'+_esc(Math.round(readiness))+'%</span>'; return h+'</div>'; }
...
function _renderTableView(){ var rows=_filteredRows(), h='<section class="sj-panel"><div class="sj-table-wrap"><table class="sj-table"><thead><tr><th data-sort="risk">'+_t('Rủi ro','Risk')+'</th><th data-sort="order_number">'+_t('Mã đơn','Order #')+'</th><th data-sort="type">'+_t('Loại','Type')+'</th><th data-sort="source">'+_t('Nguồn','Source')+'</th><th data-sort="subject">'+_t('Mô tả','Subject')+'</th><th data-sort="phase">'+_t('Phase','Phase')+'</th><th data-sort="status">'+_t('Trạng thái','Status')+'</th><th data-sort="readiness">'+_t('Readiness','Readiness')+'</th><th data-sort="due_date">'+_t('Hạn','Due')+'</th><th data-sort="exceptions">'+_t('Ngoại lệ','Exceptions')+'</th></tr></thead><tbody>'; if(!rows.length) h+='<tr><td colspan="10" class="sj-empty-row">'+_t('Không có dữ liệu phù hợp.','No matching records.')+'</td></tr>'; rows.forEach(function(item){ var band=_bandMeta(_recordBand(item)), phase=_phaseMeta(_recordPhase(item)), st=_status(_recordType(item),item.status||''); var sourceCell=_isAiSource(item)?_aiSourcePill(item):'<span style="font-size:11px;color:var(--text-3,#6b7280)">'+_t('Thủ công','Manual')+'</span>'; h+='<tr class="sj-row sj-select-order" data-id="'+_esc(_recordId(item))+'" data-type="'+_esc(_recordType(item))+'"><td><span class="sj-band-pill" style="background:'+band.bg+';color:'+band.color+'">'+_esc(_t(band.vi,band.en))+'</span></td><td><code>'+_esc(_recordId(item))+'</code></td><td>'+_recordType(item).toUpperCase()+'</td><td>'+sourceCell+'</td><td><strong>'+_esc(_recordTitle(item))+'</strong><span class="sj-table-sub">'+_esc(_recordSupport(item))+'</span></td><td>'+_esc(_t(phase.vi,phase.en))+'</td><td><span class="sj-status" style="color:'+st.color+';background:'+_rgba(st.color,.10)+'">'+_esc(st.text)+'</span></td><td>'+(_recordReadiness(item)==null?'-':_esc(Math.round(_recordReadiness(item)))+'%')+'</td><td>'+(_recordDueDate(item)?_esc(_fmtDate(_recordDueDate(item))):'-')+'</td><td>'+_esc(_recordExceptionCount(item))+'</td></tr>'; }); return h+'</tbody></table></div></section>'; }
...
/* PR #5 — Chain breadcrumb in the order detail panel.
 * Renders the upstream origin chain: AEOI Case → CPO → SO → JO → WO.
 * Steps are clickable when we have a navigable id; missing steps are
 * greyed out so the operator still sees what the chain *should* look
 * like. Source_record_id is the intake_no (e.g. INT-2026-000012). */
function _renderChainBreadcrumb(o, type){
  var hasAi = _isAiSource(o);
  // Resolve each link in the chain.
  var aiRef     = o.source_record_id || '';
  var cpoNumber = o.customer_po_number || o.customer_po || '';
  var cpoId     = o.customer_po_id || '';
  var soNumber  = o.so_number || (type==='so' ? _recordId(o) : '');
  var joNumber  = o.jo_number || (type==='jo' ? _recordId(o) : (type==='wo' ? o.jo_number : ''));
  var woNumber  = o.wo_number || (type==='wo' ? _recordId(o) : '');
  // Bail out if we have *no* chain info beyond the current record.
  if (!hasAi && !cpoNumber && !joNumber && !woNumber && !(type==='jo' && soNumber) && !(type==='wo' && soNumber)) return '';
  function step(label, value, kind, tone){
    if (!value) return '<div class="sj-chain-step muted" style="opacity:.45"><small>'+_esc(label)+'</small><strong style="font-size:11px;color:var(--text-3,#6b7280)">—</strong></div>';
    var clickable = (kind==='so' && value) || (kind==='jo' && value) || (kind==='wo' && value) || (kind==='ai' && value);
    var bg = tone||'#f3f4f6', fg = '#111';
    if (kind==='ai')  { bg='#dbeafe'; fg='#1e40af'; }
    if (kind==='cpo') { bg='#fef3c7'; fg='#92400e'; }
    if (kind==='so')  { bg='#dcfce7'; fg='#14532d'; }
    if (kind==='jo')  { bg='#e0e7ff'; fg='#3730a3'; }
    if (kind==='wo')  { bg='#fae8ff'; fg='#86198f'; }
    var attr = clickable ? ' style="cursor:pointer"' : '';
    var dataAttr = clickable && kind!=='ai' && kind!=='cpo' ? ' data-chain-id="'+_esc(value)+'" data-chain-type="'+kind+'"' : '';
    return '<button type="button" class="sj-chain-step"'+attr+dataAttr+' style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;padding:6px 10px;border:1px solid #e5e7eb;border-radius:6px;background:'+bg+';color:'+fg+';min-width:120px;cursor:'+(clickable?'pointer':'default')+'"><small style="font-size:9px;text-transform:uppercase;letter-spacing:.5px;opacity:.7">'+_esc(label)+'</small><strong style="font-size:12px;font-family:ui-monospace,monospace">'+_esc(value)+'</strong></button>';
  }
  function arrow(){ return '<span style="color:#9ca3af;font-size:14px">→</span>'; }
  var parts = [];
  if (hasAi) { parts.push(step(_t('Email AEOI','AEOI Email'), aiRef, 'ai')); parts.push(arrow()); }
  if (cpoNumber || cpoId) { parts.push(step(_t('Customer PO','Customer PO'), cpoNumber || cpoId, 'cpo')); parts.push(arrow()); }
  if (soNumber) { parts.push(step('SO', soNumber, 'so')); }
  if (joNumber) { if (soNumber) parts.push(arrow()); parts.push(step('JO', joNumber, 'jo')); }
  if (woNumber) { if (joNumber) parts.push(arrow()); parts.push(step('WO', woNumber, 'wo')); }
  if (parts.length === 0) return '';
  var titleText = hasAi
    ? _t('Chuỗi nguồn gốc — đơn này bắt đầu từ AEOI Intake','Origin chain — order originated from AEOI Intake')
    : _t('Chuỗi đơn hàng','Order chain');
  var note = hasAi
    ? '<p style="margin:6px 0 0 0;font-size:11px;color:var(--text-3,#6b7280)">'+_t('Bấm vào bước bất kỳ để mở chi tiết. Mã AEOI ('+_esc(aiRef||'?')+') liên kết với hộp thư extraction.','Click any step to open it. The AEOI ref ('+_esc(aiRef||'?')+') links back to the extraction inbox.')+'</p>'
    : '';
  return '<section class="sj-sec sj-chain-sec"><div class="sj-sec-head"><h4>'+_esc(titleText)+'</h4></div><div class="sj-chain" style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;padding:8px 0">'+parts.join('')+'</div>'+note+'</section>';
}
...
function _showDetail(id,type){
  ...
  _api('order_detail',{ order_id:id, order_type:type }).then(function(res){
    ...
    html+=_renderChainBreadcrumb(o,type);
    ...
    Array.prototype.forEach.call(body.querySelectorAll('.sj-chain-step[data-chain-id]'), function(btn){ btn.onclick=function(){ var cid=btn.getAttribute('data-chain-id'), ctype=btn.getAttribute('data-chain-type'); if(cid&&ctype) _showDetail(cid,ctype); }; });
  });
}
