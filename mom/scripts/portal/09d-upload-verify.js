...
function ensureStyles(){
  ...
  style.textContent = [
    ...
    '.uv2-card{background:var(--bg-surface,#fff);border:1px solid var(--border,#e2e8f0);border-radius:18px;box-shadow:0 10px 24px rgba(15,23,42,.05);padding:18px}',
    '.uv2-head{display:flex;justify-content:space-between;align-items:flex-start;gap:var(--space-3,12px);flex-wrap:wrap}.uv2-head h2{margin:0;font-size:18px;color:var(--text-primary,#0c2d48)}.uv2-head p{margin:var(--space-1,4px) 0 0;font-size:var(--text-xs,.75rem);color:var(--text-secondary,#64748b);line-height:1.6}',
    '.uv2-search,.uv2-dropzone{width:100%;border:1px solid var(--border,#d1d5db);border-radius:14px;box-sizing:border-box}.uv2-search{padding:10px var(--space-3,12px);font-size:13px;font-family:inherit}.uv2-search:focus{outline:none;border-color:var(--brand-2,#1565c0);box-shadow:0 0 0 3px rgba(21,101,192,.12)}',
    '.uv2-list{display:grid;gap:10px;margin-top:14px;max-height:74vh;overflow:auto;padding-right:var(--space-1,4px)}.uv2-row{border:1px solid var(--border,#e2e8f0);border-radius:16px;padding:14px;cursor:pointer;transition:all .16s;background:var(--bg-surface,#fff)}.uv2-row:hover{border-color:#93c5fd;box-shadow:0 8px 20px rgba(21,101,192,.08)}.uv2-row.active{border-color:var(--brand-2,#1565c0);background:#eff6ff;box-shadow:0 0 0 3px rgba(21,101,192,.08)}',
    '.uv2-record{font-family:Consolas,monospace;font-size:var(--text-xs,.75rem);font-weight:800;color:var(--text-primary,#0f172a)}.uv2-meta{font-size:11px;color:var(--text-secondary,#64748b);line-height:1.55;margin-top:var(--space-2,8px)}',
    '.uv2-dropzone{padding:28px 18px;border-style:dashed;background:var(--bg-surface-alt,#f8fafc);text-align:center;cursor:pointer;transition:all .16s}.uv2-dropzone.drag,.uv2-dropzone:focus{border-color:var(--brand-2,#1565c0);background:#eff6ff;outline:none;box-shadow:0 0 0 3px rgba(21,101,192,.12)}.uv2-dropzone strong{display:block;font-size:14px;color:var(--text-primary,#0c2d48)}.uv2-dropzone p{margin:var(--space-2,8px) 0 0;font-size:var(--text-xs,.75rem);color:var(--text-secondary,#64748b);line-height:1.65}',
    '.uv2-queue{display:grid;gap:var(--space-3,12px);margin-top:var(--space-4,16px)}.uv2-file{border:1px solid var(--border,#e2e8f0);border-radius:16px;padding:14px;background:var(--bg-surface,#fff)}.uv2-file-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap}.uv2-file-name{font-size:13px;font-weight:800;color:var(--text-primary,#0f172a);word-break:break-word}.uv2-file-meta{font-size:11px;color:var(--text-secondary,#64748b);line-height:1.5;margin-top:6px}.uv2-badge{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;border-radius:999px;font-size:11px;font-weight:800}.uv2-badge.pass{background:#dcfce7;color:var(--green-dark,#166534)}.uv2-badge.warn{background:#ffedd5;color:#c2410c}.uv2-badge.fail{background:#fee2e2;color:#b91c1c}.uv2-badge.pending{background:var(--border,#e2e8f0);color:#334155}',
    '.uv2-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:10px;margin-top:var(--space-3,12px)}.uv2-cell{border:1px solid var(--border,#e2e8f0);border-radius:var(--radius-lg,12px);padding:10px var(--space-3,12px);background:var(--bg-surface-alt,#f8fafc)}.uv2-cell small{display:block;font-size:10px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--text-secondary,#64748b);margin-bottom:5px}.uv2-cell strong{display:block;font-size:var(--text-xs,.75rem);color:var(--text-primary,#0f172a);line-height:1.45;word-break:break-word}',
    '.uv2-actions{display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;margin-top:var(--space-4,16px)}.uv2-btn{height:42px;border:none;border-radius:var(--radius-lg,12px);padding:0 var(--space-4,16px);font-size:13px;font-weight:800;cursor:pointer;display:inline-flex;align-items:center;gap:var(--space-2,8px)}.uv2-btn.primary{background:var(--brand-2,#1565c0);color:var(--text-inverse,#fff)}.uv2-btn.secondary{background:#eef2f7;color:#334155}.uv2-btn.ghost{background:var(--bg-surface,#fff);border:1px solid var(--border,#d1d5db);color:#334155}',
    '.uv2-empty{padding:var(--space-5,20px);border:1px dashed var(--border,#cbd5e1);border-radius:16px;background:var(--bg-surface-alt,#f8fafc);text-align:center;color:var(--text-secondary,#64748b);font-size:var(--text-xs,.75rem);line-height:1.6}.uv2-empty strong{display:block;color:var(--text-primary,#0c2d48);margin-bottom:var(--space-1,4px)}',
...
function renderFileItem(item){
  ...
  var duplicates = Array.isArray(verification.duplicate_candidates) ? verification.duplicate_candidates : [];
  var duplicateHtml = duplicates.length
    ? '<div class="uv2-file-meta" style="color:var(--red-light,#b91c1c)"><strong>' + esc(t('Trùng nội dung đã tiếp nhận:', 'Duplicate content already received:')) + '</strong> ' + esc(duplicates.map(function(d){ return ['R' + (d.version || '?'), d.uploaded_at || '', d.stored_filename || ''].filter(Boolean).join(' · '); }).join(' | ')) + '</div>'
    : '';
  return '<div class="uv2-file" data-file-id="' + esc(item.id) + '"><div class="uv2-file-head"><div><div class="uv2-file-name">' + esc(item.file.name) + '</div><div class="uv2-file-meta">' + esc(formatSize(item.file.size)) + '</div></div><span class="uv2-badge ' + badgeClass + '">' + esc(status === 'verified' ? t('Hợp lệ', 'Verified') : status === 'warning' ? t('Cảnh báo cần QA xử lý', 'Warning requires QA resolution') : status === 'rejected' ? t('Từ chối', 'Rejected') : t('Đang kiểm tra', 'Inspecting')) + '</span></div><div class="uv2-grid"><div class="uv2-cell"><small>' + esc(t('Mã hồ sơ', 'Record ID')) + '</small><strong>' + esc((allocation && allocation.record_id) || metadata.issued_record_id || '—') + '</strong></div><div class="uv2-cell"><small>' + esc(t('Biểu mẫu', 'Form')) + '</small><strong>' + esc(metadata.form_code || (allocation && allocation.form_code) || '—') + '</strong></div><div class="uv2-cell"><small>' + esc(t('Phiên bản mẫu', 'Template version')) + '</small><strong>' + esc(metadata.form_version || metadata.form_revision || (allocation && allocation.form_revision) || '—') + '</strong></div><div class="uv2-cell"><small>' + esc(t('Phiên nhận hiện tại', 'Current receipt')) + '</small><strong>' + esc(allocation && allocation.receipt_version ? ('R' + allocation.receipt_version) : t('Chưa có', 'None')) + '</strong></div></div>' + ((verification.warnings || []).length ? '<div class="uv2-file-meta" style="color:var(--amber-light,#c2410c)">' + esc((verification.warnings || []).join(', ')) + '</div>' : '') + ((verification.issues || []).length ? '<div class="uv2-file-meta" style="color:var(--red-light,#b91c1c)">' + esc((verification.issues || []).join(', ')) + '</div>' : '') + duplicateHtml + '</div>';
}
...
window._renderUploadVerify = function(schemas, entries, container){
  ...
  function bind(){
    ...
    var submitAll = document.getElementById('uv2-submit-all'); if(submitAll) submitAll.onclick = function(){
      var valid = state.files.filter(function(item){ return item.inspect && item.inspect.ok && item.inspect.verification && item.inspect.verification.status === 'verified'; });
      if(!valid.length){ toast(t('Không có tệp Excel đã xác minh để tiếp nhận. Cảnh báo và trùng lặp phải được xử lý trước.', 'There is no verified workbook to receive. Warnings and duplicates must be resolved first.'), 'warn'); return; }
