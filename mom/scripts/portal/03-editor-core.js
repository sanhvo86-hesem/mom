...
function edRefreshQmsFieldTypeUi(){
  ...
  if(!source){
    preview.innerHTML = '<div class="ed-qms-prop-note">Chọn nguồn dữ liệu nền để xem nội dung droplist.</div>';
    return;
  }
  var items = edQmsLookupPreview(source);
  if(!items.length){
    preview.innerHTML = '<div class="ed-qms-prop-note">Chưa có bản ghi trong nguồn dữ liệu này.</div>';
    return;
  }
...
function edEditQmsBlockProperties(block){
  ...
  if(isField){
    ...
    root.innerHTML =
      ...
      '<div class="ed-qms-prop-grid"><div><label>API endpoint</label><input id="ed-qms-field-api" value="' + _edEscapeHtml(fp.apiEndpoint || '') + '" placeholder="/mom/api.php?action=..."></div><div><label>' + (vi ? 'Phương thức API' : 'API method') + '</label><select id="ed-qms-field-api-method">' +
