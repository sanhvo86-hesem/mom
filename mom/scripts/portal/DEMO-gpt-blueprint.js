...
var TITLE={vi:'Module Studio',en:'Module Studio'};
var SUBTITLE={
...
function _submitCreate(){
  ...
  state.store.unshift(item); state.page.offset=0; state.detail=_clone(item); _load('detail'); _toast(_t('Đã tạo blueprint mới từ Module Studio.','New blueprint created from the Module Studio.'),'success');
}
...
function _handleClick(e){
  ...
  if(action==='refresh'){ _load(state.activeTab); _toast(_t('Đã làm mới dữ liệu demo của Module Studio.','Module Studio demo data refreshed.'),'success'); return; }
