...
| Final merge gate / publication orchestrator | Full generated-artifact pipeline is runnable from committed sources. | `CLOSED_BY_FINAL_PHASE` | `canonical_publication_orchestrator.py` passes end-to-end after restoring `orphan-resolution.json` and `data-fields-part2.json` and mapping `traceability_*` tables. | Not open. | No | None. |
...
| Full publication equals schema authority | Registry publication covers every schema table. | `PRODUCT_DECISION_REQUIRED` / `MIGRATION_ROLLOUT_REQUIRED` | Schema authority reports 772 schema tables and 760 registry tables. | Requires migration/publication scope decision. | No | Keep explicit delta. |
