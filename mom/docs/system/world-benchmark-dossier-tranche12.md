...
|---|---|---|---|---|
| ISA-95 / IEC 62264: explicit enterprise/control boundary and planning/execution separation. Sources: `isa.org` ISA-95 pages. | Current docs and services separate planning, execution, analytics, and control authority in several places. | Broad ISA-95 alignment claims are unproven where runtime registry artifacts are absent. | Keep execution truth in service paths and make generated registry artifacts match code reality. | Partly: docs corrected, no broad rewrite. |
| NIST SP 800-82 Rev. 3: OT-aware segmentation, monitoring, recovery, and safety-conscious security. Source: NIST SP 800-82 Rev. 3. | No direct machine-control behavior was added. Health and fallback signals are stronger after this run. | Full OT readiness remains unproven without deployed segmentation and OT runbook evidence. | Add operational probes and OT deployment controls without adding machine-control behavior. | Partly: logging, queue, audit, and trace proof improved. |
...
| OpenTelemetry: traces, metrics, logs, baggage, and context propagation share a context model. Source: OpenTelemetry spec pages. | `SliceObservability` emits trace/correlation/request IDs and now resets per request. | Full OTel SDK-native instrumentation is unproven; current implementation is bespoke structured logging. | Adopt or bridge to standard context propagation and cross-signal correlation. | Partly: request-scoped context fixed. |
...
The coordinator chose observability / proof-layer hardening as the highest-leverage new improvement after local code-fixable inherited defects were addressed, because it reduces false confidence across OT readiness, audit evidence, queue fallback recovery, and incident correlation without broad framework rewrite.
