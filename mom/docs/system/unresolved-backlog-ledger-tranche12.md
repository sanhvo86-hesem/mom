...
|---|---|---|---|---|---|---|---|
| Publication registry artifact set | Prior publication / foundation governance prompts, `verify_publication_truth.py` | Checked-in `mom/data/registry` artifacts prove publication truth | `BLOCKED_EXTERNAL` after coordinator verification; Agent 6 still labels it `FIX_NOW` | `python3 mom/tools/registry/canonical_publication_orchestrator.py` fails on missing `table-registry.json`, `data-fields.json`, wave policy files; schema sync fails on missing `DB_PASSWORD`; no local worktree contains a checked-in `mom/data/registry` source tree | Required source artifacts and environment credentials are absent | No in this workspace without source artifacts or credentials | Recorded as blocker; docs corrected so no green claim remains |
...
| Admin-tab authority and config persistence | `admin-tabs-international-standard-gap-assessment-2026-04-09.md` | Backend-authoritative admin config persistence and policy enforcement | `PARTIAL` | Current alias routing is already fixed; pass-2 correction: the temp-file-only fragility claim is overstated because helpers have direct-write fallback | Full authoritative persistence requires broader admin control-plane migration and product scope | Partly | No broad admin rewrite in this run; false subclaims about raw `admin_users_list` and temp-file-only writes are invalid |
...
| Logging health false green | Agent 5 pass 1 and pass 2 | Readiness must not report Loki healthy before proof, and must not fail before an actual push result | `FIX_NOW -> CLOSED in workspace tests` | `LogTransport` previously trusted URL syntax; new health exposes `loki_configured`, `loki_verified`, `loki_probe_state`; pass-2 fix reports configured/unverified as `loki_available=null` until push result | Was open because URL validation was treated as availability; pass 2 caught readiness overcorrection | Yes | Fixed with regression test |
| Observability request context | Agent 5 pass 1 | Trace/correlation/request IDs must be request-scoped | `FIX_NOW -> CLOSED in workspace tests` | `SliceObservability::beginRequest()` resets trace context; `api/index.php` calls it during boot | Was open because singleton IDs could persist in long-lived workers | Yes | Fixed with regression test |
| File queue fallback DLQ / reconciliation | Agent 5 pass 1 | File fallback distinguishes retry from poison messages | `FIX_NOW -> CLOSED in workspace tests` | `QueueService` now tracks file attempts, writes `.dead-letter.jsonl`, and reports backlog/dead-letter/reconciliation health | Was open because failed messages stayed ordinary backlog | Yes | Fixed with regression test |
| Legacy audit file sink visibility | Agent 5 pass 1 | Legacy audit file sink must not be hidden split authority | `FIX_NOW -> CLOSED in workspace tests` | Health exposes `legacy_audit_file_sink`; readiness degrades when enabled; structured audit metadata records the flag | Was open because env-enabled file copy had no probe | Yes | Fixed with regression test |
