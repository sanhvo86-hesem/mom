/**
 * HESEM MOM Portal — PWA Initialization
 * =======================================
...
(function () {
  ...
  function showUpdateNotification() {
    ...
    const existing = document.getElementById('mom-update-banner');
    if (existing) existing.remove();

    const banner = document.createElement('div');
    banner.id = 'mom-update-banner';
    banner.setAttribute('role', 'alert');
    banner.innerHTML = `
      <span>Phiên bản mới đã sẵn sàng.</span>
      <button id="mom-update-btn">Cập nhật ngay</button>
      <button id="mom-update-dismiss" aria-label="Đóng">&times;</button>
    `;
    ...
    document.getElementById('mom-update-btn').addEventListener('click', () => {
    ...
    document.getElementById('mom-update-dismiss').addEventListener('click', () => {
      banner.remove();
    });
  }
  ...
  function showInstallBanner() {
    ...
    if (sessionStorage.getItem('mom_install_dismissed') === '1') return;

    const existing = document.getElementById('mom-install-banner');
    if (existing) return; // Already shown.

    const banner = document.createElement('div');
    banner.id = 'mom-install-banner';
    banner.innerHTML = `
      <div class="mom-install-inner">
        <span>Cài đặt HESEM MOM để truy cập nhanh hơn và dùng offline.</span>
        <button id="mom-install-btn">Cài đặt</button>
        <button id="mom-install-dismiss" aria-label="Đóng">&times;</button>
      </div>
    `;
    banner.style.cssText = `
    ...
    innerStyle.textContent = `
      .mom-install-inner{display:flex;align-items:center;gap:12px;flex-wrap:wrap}
      #mom-install-btn{padding:8px 20px;background:#e94560;color:#fff;border:none;border-radius:8px;font-weight:600;cursor:pointer;font-size:13px}
      #mom-install-btn:hover{background:#d63850}
      #mom-install-dismiss{background:none;border:none;color:rgba(255,255,255,.6);font-size:20px;cursor:pointer;padding:4px}
    `;
    banner.appendChild(innerStyle);
    document.body.appendChild(banner);

    document.getElementById('mom-install-btn').addEventListener('click', async () => {
      await promptInstall();
      banner.remove();
    });

    document.getElementById('mom-install-dismiss').addEventListener('click', () => {
      banner.remove();
      // Remember dismissal for this session.
      sessionStorage.setItem('mom_install_dismissed', '1');
    });
  }

  function hideInstallBanner() {
    const el = document.getElementById('mom-install-banner');
    if (el) el.remove();
  }
  ...
  function handleSwMessage(event) {
    ...
    switch (type) {
      case 'SYNC_COMPLETE':
        ...
        if (window.momSyncManager) {
          window.momSyncManager.syncAll();
        }
        break;
  ...
  async function initPWA() {
    console.log('[PWA] Initializing HESEM MOM PWA...');
    ...
    if (window.momOfflineStore) {
      try {
        await window.momOfflineStore.init();
    ...
    if (window.momSyncManager) {
      try {
        await window.momSyncManager.init();
    ...
    if (window.currentUser && window.momOfflineStore) {
      try {
        await window.momOfflineStore.cacheUserData(window.currentUser);
      } catch (e) {
        // Non-critical.
      }
    }
