...
This directory contains ordered PostgreSQL migration files that compose the full HESEM MOM/ERP/MES/eQMS database schema. The migration set now spans `001` through `127`, and the current authority registry tracks the full platform contract across ERP, MOM, MES, APS, PLM, WMS, HCM, CRM, TMS, trade, SRM, tooling, EHS, finance, quality, maintenance, analytics, runtime idempotency, canonical manufacturing event backbone, trusted release record, connected governance, eQMS control-plane domains, release authority hardening, e-signature challenges, and change/genealogy closure guards.
...
| `107`-`127` | Control-plane execution tables, cutover hardening, AI advisory boundary comments, security/index/constraint hardening, audit columns, source-status authority constraints, genealogy/runtime ontology and 5M context filters, e-signature challenge authority, change resulting-object scope enforcement, and single-current as-manufactured snapshot guard |
...
Each migration file contains rollback instructions in comments at the bottom of the file where rollback is safe. To rollback, execute the rollback SQL statements in reverse order (`127` first, then `126`, and so on). Destructive rollback of governance constraints must be treated as a controlled change and not run on production without a released change order.
...
- The current migration set contains `001` through `127`, with the internal `schema_migrations` ledger, the authoritative idempotency replay ledger, canonical manufacturing operational event ledger, trusted release record spine, eQMS control-plane tables, connected governance revision/training/execution tables, e-signature challenge authority, and change/genealogy closure constraints. This is the ERP+MOM/MES/eQMS system contract authority used by Data Schema.
