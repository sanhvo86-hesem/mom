...
$phpFiles = @(
  ...
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\EpicorInboundWorker.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\EpicorTransportAdapter.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\OutboxWorker.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\ScheduledJobs.php"),
  ...
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\MtconnectPollingService.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\ScheduledJobs.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\UploadHardeningService.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\api\services\WorkflowEngine.php"),
  (Join-Path $RepoRoot "01-QMS-Portal\scripts\run_scheduled_job.php"),
  ...
  (Join-Path $RepoRoot "01-QMS-Portal\scripts\run_mtconnect_poll_cycle.php")
)
...
$schemaFiles = @(
  ...
  (Join-Path $RepoRoot "01-QMS-Portal\database\migrations\030_mes_realtime_notify.sql"),
  (Join-Path $RepoRoot "01-QMS-Portal\database\migrations\031_mes_dpp_energy_cost_foundations.sql")
)
...
$schemaCommand = @"
rg -n 'mes_connectivity_adapters|mes_connectivity_events|mes_alarm_catalog|mes_alarm_playbooks|mes_nc_release_packages|mes_nc_download_receipts|mes_tool_preset_offsets|mes_tool_assemblies|mes_erp_sync_runs|mes_erp_reconciliation_exceptions|integration_system|sync_direction|sync_status|completed_at|records_received|records_processed|exception_status|expected_value|actual_value|is_acknowledged|acknowledged_at|related_job_number|v_mes_adapter_health|v_mes_nc_release_readiness|create_hypertable|timescaledb\\.continuous|mes_machine_telemetry_hourly|mes_downtime_daily|mes_tool_life_daily|add_retention_policy|add_compression_policy|mes_machine_state_events|mes_machine_telemetry|mes_inline_measurements|pg_notify|notify_mes_telemetry|notify_mes_alarm|notify_mes_downtime' "$($schemaFiles -join '" "')"
"@
Invoke-Checked "MES schema foundation lookup" $schemaCommand

Write-Step "MES variable coverage"
$variableCommand = @"
rg -n 'mes_alarm_management|mes_nc_release_governance|mes_tool_offset_governance|mes_material_genealogy_governance|mes_shift_handover_governance|erp_mes_integration_governance|mes_digital_product_passport|mes_energy_management|mes_cost_governance|material_issue_id_mes|genealogy_snapshot_id_mes|shift_handover_id_mes|handover_acknowledged_at_mes|epicor_sync_run_id_erp|epicor_sync_domain_erp|epicor_outbox_event_id_erp|dpp_id_mes|dpp_origin_country_mes|dpp_carbon_footprint_mes|energy_snapshot_id_mes|machine_power_kw_mes|energy_per_unit_kwh_mes|cost_tracking_id_mes|standard_cost_total_mes|actual_cost_total_mes|cost_variance_pct_mes|cost_per_good_unit_mes' "$variableLibraryFile"
"@
Invoke-Checked "MES variable lookup" $variableCommand

Write-Step "MES wiring"
$wiringCommand = @"
rg -n 'page-mes|page-exceptions|14-mes-control-center|14-exception-dashboard|_renderMesControlCenter|_renderExceptionDashboard|navigateTo\(''exceptions''\)|data-page=\"mes\"|mes_snapshot|mes_stream|mes_cutover_audit|mes_foundation_snapshot|mes_alarm_snapshot|mes_nc_release_snapshot|mes_tool_offset_snapshot|mes_dpp_snapshot|mes_energy_snapshot|mes_cost_snapshot|mes_connector_snapshot|mes_machine_signal_upsert|mes_connector_ingest|mes_mtconnect_poll_once|mes_mtconnect_poll_batch|mes_mtconnect_poll_machine|mes_mtconnect_poll_batch_runtime|mes_mtconnect_poll_due_state|mes_mtconnect_poll_url|mes_http_fetch_text|mes_pg_open_notify_stream|mes_pg_wait_for_notify|mes_adapter_event_append|mes_alarm_runtime_upsert|mes_alarm_acknowledge|mes_alarm_escalate|mes_alarm_clear|mes_nc_download_receipt|mes_tool_offset_upsert|mes_dpp_upsert|mes_energy_snapshot_upsert|mes_cost_tracking_upsert|mes_material_issue|mes_genealogy_snapshot|mes_shift_snapshot|mes_shift_handover_submit|mes_shadow_status|epicor_sync_snapshot|epicor_transport_health|epicor_sync_run_upsert|epicor_reconciliation_upsert|epicor_outbox_upsert|epicor_outbox_process|epicor_inbound_process|exception_dashboard|exception_detail|program_mismatches|program_handshake_queue|program_release_queue|tool_readiness_queue|program_release_risk|tool_readiness_risk|alarm_ack_gaps|review_sla_gaps|review_sla_queue|evidence_sla_notifications_run|material_genealogy_gaps|shift_handover_gaps|adapter_governance_risk|alarm_hotspots|nc_download_mismatches|tool_offset_risk|launch_blocker_hotspots|launch_blocker_queue|launch_blockers|adapter_governance_queue|alarm_hotspot_queue|alarm_ack_queue|nc_download_mismatch_queue|tool_offset_queue|operator_qualification_gaps|material_trace_gaps|material_genealogy_queue|shift_handover_queue|connector_governance_gaps|shadow_sync_failures|primary_read_fallbacks|primary_read_queue|primary_read_status|primary_reads|cutover_audit|cutover_drift_entities|build_runtime_cutover_audit|operator_qualification_queue|material_trace_queue|connector_guard_queue|current_shift|load_mes_shift_patterns|mes_build_alarm_ack_queue|mes_material_genealogy_overlay|mes_merge_material_trace_with_genealogy|mes_build_material_genealogy_queue|mes_build_shift_handover_queue|mes_build_dpp_queue|mes_build_energy_queue|mes_build_cost_variance_queue|dpp_queue|energy_queue|cost_variance_queue|dpp_readiness_gaps|energy_tracking_gaps|cost_variance_risk|shadow_status|connector_ingest_status|runtime_mode|stream_mode|live_event_source|notify_channels|polling_fallback|skipped_count|last_skipped_at|downtime_governance_gaps|wo_launch_blocked|adapter_governance_failed|machine_alarm_active|nc_download_not_ready|tool_offset_not_ready|material_genealogy_incomplete|shift_handover_incomplete|stale_signal_timestamp|mes_wo_transition_guard|oee_timeline|downtime_pareto|mes_wo_report_progress|mes_downtime_create|mes_maintenance_create|mes_tooling_upsert|runMtconnectPollingCycle|runEvidenceReviewSlaNotifications|runEpicorInboundSync|runEpicorOutboxWorker|run_scheduled_job|epicor_inbound_sync|epicor_outbox_worker|data-open-signal-bridge|data-poll-mtconnect|mes-pull-epicor|mes-push-epicor|mes-run-sla|pollMtconnectBatch|processEpicorInbound|processEpicorOutbox|runEvidenceSlaNotifications|data-open-adapter-event|data-open-forms-review|data-alarm-action|data-shift-handover|data-material-issue|data-genealogy-snapshot|pollMtconnect|openAdapterEventModal|openAlarmGovernanceModal|openShiftHandoverModal|openMaterialIssueModal|openGenealogySnapshotModal|nc_program_releases|mes_connectivity_adapters|mes_alarm_catalog|mes_alarm_playbooks|tool_assemblies|mes_connectivity_events|machine_alarm_events|nc_download_receipts|mes_tool_preset_offsets|material_consumption|part_genealogy|shift_handover|dpp_passports|energy_snapshots|cost_tracking|downtime_reason_codes|downtime_resolution_codes|reason_code|resolution_code|runtime_data_layer|shadow_sync_master_data_store|shadow_sync_orders_store|shadow_sync_mes_runtime_store|shadow_sync_epicor_runtime_store|observe_wo_launch_blocked|observe_primary_read|mes_signal_replay_guard|runtime_read_model_bundle|getRuntimeMasterDataStore|getRuntimeOrdersStore|getRuntimeMesRuntimeStore|getRuntimeEpicorIntegrationStore|EpicorIntegrationService|EpicorInboundWorker|EpicorTransportAdapter|OutboxWorker|epicor_integration_policy.json|epicor-runtime.json|epicor_sync_status|epicor_sync_queue|epicor_reconciliation_open|epicor_outbox_pending|load_epicor_runtime_store|save_epicor_runtime_store|integration_system|sync_direction|sync_status|completed_at|records_received|records_processed|exception_status|expected_value|actual_value|EventSource|connectStream|disconnectStream|streamEligible|streamStatus|primary_read_fallbacks|read_retry_count|read_retry_delay_ms|utf8_encoding|UTF8_VALIDATED_EXTENSIONS' "$($wiringFiles -join '" "')"
"@
