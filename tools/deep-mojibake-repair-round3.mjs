...
const REPORT_PATH = path.join(ROOT, '_reports', 'encoding_autofix_round3.csv');
...
function markerCount(s) {
  let bad = (s.match(MARKER_RE) || []).length;
  bad += (s.match(/[\u0080-\u009f]/g) || []).length;
  if (s.startsWith('\uFEFF')) bad += 1;
  return bad;
}
...
function fixLine(line) {
  if (!MARKER_TEST_RE.test(line)) return line;
  const parts = line.split(/([ \t\r\n]+|[<>"'=])/);
  let changed = false;
  for (let i = 0; i < parts.length; i++) {
    const token = parts[i];
    if (!token || DELIM_RE.test(token)) continue;
    if (!MARKER_TEST_RE.test(token)) continue;
