...
| Section | Questions must be answered | Nature |
|---|---|---|
...
- Loss of important state change points such as `HOLD`, `revalidation`, `move to next op`, `final release`.
- Make documents that are beautiful in form but weak in practical operation.

### 1.3 When are 5 ports used?
...
- minimal evidence correct,
- the right authority,
- `PASS / CONDITIONAL PASS / HOLD / FAIL` decision is clear.
...
| **IG1, IG2...** | Internal control port | Inside a specific SOP |
...
2. There is a clear owner.
3. There are measurable conditions.
...
- the action does not create a decision to open the gate,
- simple note-taking steps,
- The subcheck does not change the job status.
...
- If the old SOP has useful gate logic, it is necessary to retain the correct operational thinking before standardizing the table structure.
...
| Port name | Must be operating language, not ambiguous |
| Chair | The role has permission to hold/open ports |
| Stop | Specific, measurable, auditable |
| KPI | Has a target number or open/closed status |
| File | Only state the minimum documents required to open the gate |
...
- `Tốc độ phản ứng` as `≤ 30 phút`, `≤ 1 ca`, `≤ 24 giờ`.
...
- `Tính ổn định` as `Cpk tối thiểu`, `% on-time calibration`, `% action đóng đúng hạn`.
...
3. **Reaction Trigger**: how much deviation to hold the gate, open escalation or open action.
4. **Based on final number**:
...
| Family | Commonly used starting KPIs |
|---|---|
...
- ❌ Write a stop like "quality assurance", "according to requirements".
- ❌ Using owner does not have permission to open the port.
...
- change roles,
- change area/cell/step,
- change system state,
- change main resource (`machine`, `program`, `fixture`, `gage`, `material`),
- control decisions arise,
- arise important evidence/records,
- generates `revalidation`, `containment`, `handover`.

### 3.3 Recommended practical step range
...
| Details step by step | Each step must have a corresponding heading |

**Note:** This rule is **not related** to the number of IGs in Section 6.

### 3.5A Flowchart graphic rules
...
- There should be two layers of protection:
  - HTML generates a built-in inline style for bubbles.
  - The global CSS has a fallback palette based on position so that old files or handwritten files do not lose color.
- With automatic generation SOP, inline styles should be generated simultaneously for `flow-step`, `flow-num` and `flow-arrow`.
...
2. Step numbers increase continuously from `1...n`
...
4. Start or end an individual quality control.
...
- **Section 7** = description by **execution workflow**

So:

- an IG can cover many steps,
...
- Only when the process is really short will the number of IGs and the number of steps be accidentally equal.

### 4.2 Good mapping example

| IG | Gate target | The detailed step is usually located under this gate |
|---|---|---|
| IG1 | Correct input lock | B1 receives, B2 reviews the request |
| IG2 | Baseline / route / package ready | B3 release package, B4 resource planning |
| IG3 | Readiness at the scene | B5 material/tool/fixture/gage ready, B6 setup ready |
...
- The 5 IGs and 5 iterations have exactly the same name.
...
- **Good reference detailed step:** `10–14`
...
- [ ] Do not use vague words for gate opening conditions
...
- [ ] The SOP is not “beautiful to present” but lacks implementation logic
