...
-- ---------------------------------------------------------------------------
-- MES-14.3 mes_erp_sync_runs / Lich su chu ky dong bo Epicor
-- ---------------------------------------------------------------------------
CREATE TABLE mes_erp_sync_runs (
...
CREATE INDEX idx_mes_erp_sync_runs_domain ON mes_erp_sync_runs (sync_domain, started_at DESC);
CREATE INDEX idx_mes_erp_sync_runs_status ON mes_erp_sync_runs (sync_status, started_at DESC);

-- ---------------------------------------------------------------------------
-- MES-14.4 mes_erp_reconciliation_exceptions / Sai lech doi soat MES-Epicor
-- ---------------------------------------------------------------------------
CREATE TABLE mes_erp_reconciliation_exceptions (
...
CREATE INDEX idx_mes_erp_recon_status ON mes_erp_reconciliation_exceptions (exception_status, detected_at DESC);
CREATE INDEX idx_mes_erp_recon_domain ON mes_erp_reconciliation_exceptions (sync_domain, detected_at DESC);
...
-- ============================================================================
-- SECTION 20E: WORLD-CLASS DPP / ENERGY / COST FOUNDATIONS
-- ============================================================================

CREATE TABLE mes_dpp_passports (
...
COMMENT ON TABLE mes_dpp_passports IS 'Governed digital product passport records for shipment-grade traceability and sustainability disclosure.';
CREATE UNIQUE INDEX idx_mes_dpp_job ON mes_dpp_passports (job_number);
CREATE INDEX idx_mes_dpp_item ON mes_dpp_passports (item_id, part_rev);

CREATE TABLE mes_energy_snapshots (
...
COMMENT ON TABLE mes_energy_snapshots IS 'Governed shift or interval energy intensity snapshots aligned to ISO 50001-style monitoring.';
CREATE INDEX idx_mes_energy_equipment_ts ON mes_energy_snapshots (equipment_id, captured_at DESC);
CREATE INDEX idx_mes_energy_wo ON mes_energy_snapshots (work_order_number, captured_at DESC);

CREATE TABLE mes_cost_tracking (
...
COMMENT ON TABLE mes_cost_tracking IS 'Governed WO-level actual-versus-standard costing snapshots for real-time variance control.';
CREATE UNIQUE INDEX idx_mes_cost_wo ON mes_cost_tracking (work_order_number);
CREATE INDEX idx_mes_cost_status ON mes_cost_tracking (cost_status, captured_at DESC);
