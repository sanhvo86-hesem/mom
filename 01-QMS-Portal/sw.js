/**
 * HESEM QMS Portal — Service Worker
 * ==================================
...
const PRECACHE_URLS = [
  ...
  '/01-QMS-Portal/scripts/portal/03-editor-core.js',
  '/01-QMS-Portal/scripts/portal/03-commands-layout.js',
  '/01-QMS-Portal/scripts/portal/03-commands-link.js',
  '/01-QMS-Portal/scripts/portal/03-commands-table.js',
  '/01-QMS-Portal/scripts/portal/04-workflow-actions.js',
  '/01-QMS-Portal/scripts/portal/05-workflow-panel.js',
  ...
  '/01-QMS-Portal/scripts/gateway-landing.js',
  '/01-QMS-Portal/assets/js/offline-store.js',
  '/01-QMS-Portal/assets/js/sync-manager.js',
...
const SYNC_TAG_FORMS = 'hesem-qms-form-sync';

/** Background sync tag for offline NCR submissions. */
const SYNC_TAG_NCR = 'hesem-qms-ncr-sync';
...
async function syncOfflineSubmissions(tag) {
  try {
    ...
    for (const item of allItems) {
      try {
        const response = await fetch('/01-QMS-Portal/api.php', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            action: 'form_submit',
...
self.addEventListener('push', (event) => {
  ...
  let data = {
    ...
    icon: '/01-QMS-Portal/assets/icons/icon-192x192.png',
    badge: '/01-QMS-Portal/assets/icons/icon-72x72.png',
    tag: 'qms-notification',
    data: { url: '/01-QMS-Portal/portal.html' },
  };
  ...
  const options = {
    body: data.body,
    icon: data.icon || '/01-QMS-Portal/assets/icons/icon-192x192.png',
    badge: data.badge || '/01-QMS-Portal/assets/icons/icon-72x72.png',
    tag: data.tag || 'qms-notification',
    vibrate: [200, 100, 200],
    requireInteraction: data.requireInteraction || false,
    data: data.data || { url: '/01-QMS-Portal/portal.html' },
    actions: data.actions || [
      { action: 'open', title: 'Xem', icon: '/01-QMS-Portal/assets/icons/icon-72x72.png' },
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = event.notification.data?.url || '/01-QMS-Portal/portal.html';
    ...
    self.clients.matchAll({ type: 'window', includeUncontrolled: true })
      .then((clientList) => {
        ...
        for (const client of clientList) {
          if (client.url.includes('/01-QMS-Portal/') && 'focus' in client) {
            client.postMessage({
...
/**
 * Open the hesem_qms_offline IndexedDB database.
...
function openDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open('hesem_qms_offline', 1);
