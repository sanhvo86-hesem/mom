#!/usr/bin/env bash
# ============================================================================
# HESEM QMS Portal — VPS Auto-Setup Script
...
APP_DIR="/var/www/hesem-qms"
REPO_URL="https://github.com/sanhvo86-hesem/hesemqms.git"

echo "============================================"
echo " HESEM QMS Portal — VPS Setup"
echo " Domain: $DOMAIN"
...
# HESEM QMS Portal - PostgreSQL tuning for 8GB RAM VPS
shared_buffers = '2GB'
...
cat > /etc/nginx/sites-available/hesem-qms <<'NGINX'
server {
    listen 80;
    listen [::]:80;
    server_name qms.hesem.com.vn;

    root /var/www/hesem-qms/01-QMS-Portal;
    ...
    location ~ \.php$ {
        ...
        fastcgi_param DB_NAME hesem_qms;
        fastcgi_param DB_USER qms_app;
...
sed -i "s/__DB_PASS_PLACEHOLDER__/$DB_PASS/" /etc/nginx/sites-available/hesem-qms

ln -sf /etc/nginx/sites-available/hesem-qms /etc/nginx/sites-enabled/
...
chmod -R 755 "$APP_DIR"
chmod -R 775 "$APP_DIR/01-QMS-Portal/qms-data"

# ── 8. Run Database Migrations ──────────────────────────────────────────────
echo "[8/10] Running database migrations..."
cd "$APP_DIR/01-QMS-Portal/database/migrations"
...
#!/usr/bin/env bash
set -euo pipefail
cd /var/www/hesem-qms
echo "Pulling latest code..."
git fetch origin
git reset --hard origin/main
chown -R www-data:www-data .
chmod -R 755 .
chmod -R 775 01-QMS-Portal/qms-data
...
echo ""
echo "============================================"
echo " HESEM QMS Portal — Setup Complete!"
echo "============================================"
echo ""
echo " URL:       https://$DOMAIN"
echo " IP:        103.110.87.55"
echo " App Dir:   $APP_DIR/01-QMS-Portal"
echo ""
