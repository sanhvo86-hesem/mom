...
function ensureStyles(){
  ...
  style.textContent = [
    ...
    '.uv2-dropzone{padding:28px 18px;border-style:dashed;background:#f8fafc;text-align:center;cursor:pointer;transition:all .16s}.uv2-dropzone.drag{border-color:#1565c0;background:#eff6ff}.uv2-dropzone strong{display:block;font-size:14px;color:#0c2d48}.uv2-dropzone p{margin:8px 0 0;font-size:12px;color:#64748b;line-height:1.65}',
    '.uv2-queue{display:grid;gap:12px;margin-top:16px}.uv2-file{border:1px solid #e2e8f0;border-radius:16px;padding:14px;background:#fff}.uv2-file-head{display:flex;justify-content:space-between;gap:10px;align-items:flex-start}.uv2-file-name{font-size:13px;font-weight:800;color:#0f172a;word-break:break-word}.uv2-file-meta{font-size:11px;color:#64748b;line-height:1.5;margin-top:6px}.uv2-badge{display:inline-flex;align-items:center;gap:6px;padding:5px 10px;border-radius:999px;font-size:11px;font-weight:800}.uv2-badge.pass{background:#dcfce7;color:#166534}.uv2-badge.warn{background:#ffedd5;color:#c2410c}.uv2-badge.fail{background:#fee2e2;color:#b91c1c}.uv2-badge.pending{background:#e2e8f0;color:#334155}',
  ...
  function bind(){
    ...
    var submitAll = document.getElementById('uv2-submit-all'); if(submitAll) submitAll.onclick = function(){
      ...
      Promise.all(valid.map(function(item){
        ...
        return window.AllocationTracker.receiveUpload(allocationId, item.file).then(function(resp){ if(resp && resp.ok && resp.allocation) linkOrderIfPossible(resp.allocation); item.receive = resp; return resp; });
      })).then(function(){ toast(t('Đã tiếp nhận xong các workbook hợp lệ.', 'All valid workbooks were received.'), 'success'); return window.AllocationTracker.getHistory({ delivery_mode:'offline', page_size:50 }); }).then(function(resp){ state.rows = (resp && Array.isArray(resp.entries)) ? resp.entries : state.rows; state.files = []; render(); }).finally(function(){ submitAll.disabled = false; });
    };
  }
};
