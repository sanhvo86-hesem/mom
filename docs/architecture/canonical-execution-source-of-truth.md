...
| AI natural-language query | `NaturalLanguageQueryService` over read-only PostgreSQL SELECTs and its schema/provenance relation registry | Conversation history in `ai_conversations` | NLQ is scoped, CSRF-protected, audited, read-only, and cannot write execution truth. Prompt schema and SQL allowlist share one registry so AI cannot reference undeclared operational tables. |
...
- AI natural-language query and RCA POST surfaces require CSRF because they write conversation/advisory history or trigger advisory processing. NLQ throttling is a shared user/hour ledger, not session-only state. NLQ schema grounding is registry-based: every readable relation must declare authority and projection role before entering the AI prompt or SQL allowlist.
