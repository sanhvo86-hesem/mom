...
- forcing all SOPs to the same control port number,
- force every SOP to the same number of detailed steps,
...
1. Each SOP must be studied individually.
2. Do not use one step model or gate model for the entire SOP set.
...
4. The flowchart of Section 7 must match absolutely the number of detailed step headings in the same Section 7.
5. Section 6 and Section 7 are two different classes:
   - Section 6 answers: where to keep, who opens the gate, what conditions to move forward.
...
10. Do not bulk upgrade multiple SOPs at the same time if the changed content touches the operational logic; Batch only with technical, graphic, CSS, linking, markup or structural hygiene errors.
11. Before editing the role, owner, authority, RACI or owner cell of the SOP, you must compare `JD thật -> role registry -> benchmark chính thức bên ngoài` and then finalize the mapping.
12. Labels like `Process Owner`, `Department Head`, `Responsible Person`, `Data Owner`, `Lead Auditor`, `Incident Commander` should not be preserved if they are acting as real authorities; Must resolve to base role, governance hat or explicit role bundle.
...
- realize the part is mechanically compressed or lacks a handing point,
...
- summary article without original source,
...
- What data in the system can truly measure that KPI?
...
2. clearly state this is **internal target inferred from benchmark + risk level**,
...
- Which number is the internal target inferred by risk?
- Reasons for tightening or loosening compared to benchmark.
...
- Which port is the condition before release,
- Which KPI measures the effectiveness of each port?
- Which KPI has a benchmark or design threshold capable of operating, not just sufficient form?

### Step 4. Finalize the actual work sequence

Before writing Section 7, you must answer:
- Which roles change where?
- which system or record changes where,
- Where can I change machine/tool/program/fixture/material/gage?
- Which points need revalidation?
- Which point creates the suspect range?
- Which point is required to be handed over?
...
- rewrite `Section 4` from the owner holding the gate and the role that has permission to block/unhold,
...
## 4. Rules for replacing Section 6 and Section 7
...
- KPIs must be readable according to the actual operating logic, for example: `100% ... trước ...`, `<= 24 giờ`, `= 0 escape`, `>= 99%`.
...
- There must not be a condition:
  - 7 step flowchart but 10 detailed steps,
  - 5 step flowchart but detailed description of 8 steps,
  - flowchart is just a shortened version that does not match the executable content.
- With automatically generated SOPs, it is also necessary to check that the bubble flowchart has the correct color palette according to the step index, not just the correct quantity.

---

## 5A. Hygiene rules for pre-release drafts

- Documents that have not been officially released for the first time always retain `V0`.
...
- If you need to keep traces of editing, use DCR, review log, script output or commit log.
...
- Only keep abbreviations when they are truly common operating standards and do not obscure the meaning.
...
- IG numbers repeat in bulk without real range,
...
- the content doesn't explain why that gate must be kept or why that step must be separated.
...
1. Have you read the old documentation yet?
...
6. Does Section 6 have owner, hold and measurable KPIs?
7. Does Section 7 demonstrate sufficient handover, revalidation, restart or containment?
8. Has Section 3 used the pattern `English (Việt)`?
9. Has the content of the document itself been prioritized in standard Vietnamese?
10. Have you replaced the correct parts `p6→p7` and `p7→p8` without accidentally deleting the next part?
...
- every time a new SOP is written,
- every time you rewrite Section 1, 2, 3, 4, 5, 6, 7 or 8,
- Every script automatically changes sections of the SOP,
- any mass standardization project in `core-standard`.
