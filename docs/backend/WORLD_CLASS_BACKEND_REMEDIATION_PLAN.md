...
| SO/JO/WO status | `engineering_ready` is present in runtime SO config/service; `wf_sales_order`, `wf_job_order`, and `wf_work_order_execution` now reference runtime status sets; SO line support tables no longer inherit header workflow; migrations `115`-`120` constrain SO/JO/production WO and singular MES WO columns. | Order-hierarchy P0 closed; all-domain generator remains roadmap hardening | Frontend/backend order workflows no longer validate against stale status-set aliases. Broader OpenAPI/frontend generator parity remains hardening before enterprise cutover. |
...
| `wf_sales_order` | Workflow/header tables now point to `sales_order_status_runtime`; support line tables have no header workflow; promotion gate rejects stale SO status-set references. | P0 closed | Generic transition validation can no longer use `sales_order_status_code` for SO workflow tables. |
...
| --- | --- | --- | --- | --- | --- |
| P0-01 Status/workflow authority | SO workflow/status drift was manually duplicated; broader all-domain status generation is still not complete. | SO P0 is closed by `sales_order_status_runtime`, migrations `115`/`116`, and `check_workflow_status_authority.php`; all-domain generator remains roadmap hardening. | `OrderService`, `OrderWorkflowService`, `mom/contracts/table-registry.json`, `WorkflowStatusAuthorityService`, SO status migrations. | Legacy SO aliases map to canonical states and stale workflow references fail promotion. | No `wf_sales_order` table references a mismatched status set; broader generator work is tracked separately. |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| P0-01 status/workflow authority | Manual SO status duplication across contracts and DB allowed `wf_sales_order` to reference the wrong vocabulary. | Canonical SO runtime status authority plus promotion gate; generated all-domain authority remains target hardening. | `OrderService`, `OrderWorkflowService`, `mom/contracts/table-registry.json`, `WorkflowStatusAuthorityService`, `check_workflow_status_authority.php`, DB checks/OpenAPI. | Migrations `115` and `116` map legacy SO aliases and add checks; unknown future aliases must enter migration hold. | Generated command transition guards. | WF-001..WF-008 plus workflow authority release gate test. | Keep legacy aliases read-only; reject unknown writes; rollback via pre-migration status snapshot. |
