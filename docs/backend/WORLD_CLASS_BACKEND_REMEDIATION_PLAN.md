...
| P0-02 Command layer | Generic CRUD and JSON services mutate business records directly. | Implement dedicated command APIs with DB transaction, idempotency key, row locks, domain gates, audit/evidence, and outbox. | New command services/controllers; deprecate Generic CRUD mutations for governed tables. | Add `idempotency_keys`, `command_audit`, `domain_events`, `outbox_events`, generated OpenAPI. | Every P0 business mutation in `DOMAIN_COMMAND_SPEC.md` is executable only through command API. |
...
| P0-02 command layer | Direct business mutations through JSON services and Generic CRUD. | Dedicated transactional command framework. | New command controllers/services; `GenericCrudController`; OpenAPI. | Add command/idempotency/audit/outbox tables. | All commands in `DOMAIN_COMMAND_SPEC.md`. | FE-001..FE-006, IDEMP suite. | Legacy endpoints remain read-only or 403/410 with replacement command links. |
...
| P0-08 master-data sync | PG rebuild omits readiness keys in JSON master data. | Complete JSON<->PG round-trip for readiness collections. | `DataLayer`, `RuntimeShadowSync`, bootstrap/sync/audit tools. | Add mapping for BOM/routing/CP/IP/traveler/gates/approvals. | MDM sync jobs plus release commands. | PG-001..PG-007. | Keep JSON snapshot as import fallback until drift report passes. |
