...
| Planning-to-execution consistency | 3.2 | 3.8 | 4.2 | JO/WO updates use allowlists, WO creation rejects terminal parent JOs, order schedule aliases remain routed to scheduling owner, and schedule slot overlap checks are enforced in both DB and JSON paths. |
...
| Reliability / idempotency / offline resilience | 3.4 | 4.2 | 4.3 | Evidence replay key contract is aligned; mobile task events and inspection replay identity are preserved; DB and JSON schedule writes share conflict guards; online mobile clock-in, clock-out, and completion support same-key replay. |
