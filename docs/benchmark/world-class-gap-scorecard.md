...
| Planning-to-execution consistency | 3.2 | 3.9 | 4.2 | JO/WO updates use allowlists, SO dates are validated, orphan order holds are rejected, WO creation rejects terminal parent JOs, order schedule aliases remain routed to scheduling owner, and schedule slot overlap checks are enforced in both DB and JSON paths. |
...
| AI / copilot architecture quality | 3.3 | 4.6 | 4.7 | Legacy AI reads now require AI read roles, feedback requires feedback/write roles, schedule metrics and JSON prediction fallback are plant-scoped, conversation fallback reads validate ID/owner, NLQ uses a shared user/hour throttle, and AI schedule apply/PM routes are explicitly advisory-only. |
...
| Reliability / idempotency / offline resilience | 3.4 | 4.3 | 4.4 | Evidence replay key contract is aligned; mobile task events and inspection replay identity are preserved; DB and JSON schedule writes share conflict guards; online mobile clock-in, clock-out, and completion support same-key replay; mobile queue defaults use the factory calendar; manufacturing-event fallback append keeps idempotency/hash-chain checks while avoiding full in-memory retention. |
| Performance / scalability | 3.0 | 3.4 | 4.1 | Mobile operator queue retrieval now uses a derived operator/date index, and file-backed manufacturing-event append scans duplicate/hash context without retaining the whole ledger. Broader mobile/offline and dashboard indexes remain staged. |
...
| H12 AI detached/unsafe | P1 | Fixed for NLQ/RCA security, schema drift, AI schedule apply/PM advisory boundaries, and schedule-write conflict symmetry; full semantic copilot registry deferred. |
...
| H16 tests too narrow | P2 | Confirmed. Focused tests cover dispatch/service behavior, but full enterprise regression still exposes existing debt in broader suites. This pass adds dispatch lifecycle coverage; DB-primary/EQMS/CNC cutovers still need broader tests. |
