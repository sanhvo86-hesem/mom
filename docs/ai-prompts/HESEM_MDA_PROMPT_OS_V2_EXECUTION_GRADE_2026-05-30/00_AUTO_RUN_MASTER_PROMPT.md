# AUTO-RUN MASTER PROMPT — HESEM MDA PROMPT OS V2

Paste this prompt into the AI agent that has repository access.
...
You must execute the prompt files in `docs/ai-prompts/HESEM_MDA_PROMPT_OS_V2_EXECUTION_GRADE_2026-05-30/prompts/` in numeric order from `P00` to `P33`.

You must not wait for the user between prompts. You must not ask which prompt comes next. The sequence matrix is authoritative.
...
4. run operational simulation;
5. repair P0/P1 defects;
6. update ledgers;
7. write the decision token;
8. only then continue to the next prompt.

If a prompt produces `REPAIR_REQUIRED`, continue repairing the same prompt. If the blocker is environment-only, produce the exact repair command, mark the prompt `BLOCKED_ENVIRONMENT`, do not claim pass, and do not execute downstream runtime-changing prompts.

## First action

Execute `prompts/P00_master_orchestrator_and_sequence_controller.md` now.
