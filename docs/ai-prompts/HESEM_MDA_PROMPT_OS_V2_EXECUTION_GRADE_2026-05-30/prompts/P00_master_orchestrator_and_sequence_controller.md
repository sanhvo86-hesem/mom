# P00 — Master orchestrator, sequence gate controller, and no-guess execution constitution
...
| --- | --- |
| Prompt ID | `P00` |
| Title | Master orchestrator, sequence gate controller, and no-guess execution constitution |
| Mode | `PLANNING_AND_GOVERNANCE_ONLY` |
| Required previous token | `NONE` |
| Unlock token | `P00_PASS_READY_FOR_NEXT` |
| Domain affected | `platform_governance` |
| Root / scope code | `MDA-SEQ` |

## Prompt objective

Create the V2 run root, sequence state, source map, decision log, traceability skeleton, and enforcement rules that force one-prompt-at-a-time execution.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `NONE` is present unless this is `P00`.
...
- `AGENTS.md`
- `.ai/AI-WORKFLOW.md`
- `docs/ai-prompts/HESEM_MDA_PROMPT_OS_V2_EXECUTION_GRADE_2026-05-30/README.md`
...
1. Read `.ai/CONVENTIONS.md`, `.ai/repo-map.json`, `AGENTS.md`, `.ai/AI-WORKFLOW.md`, and this prompt pack README before any other action.
2. Create the report root `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/` if it does not exist.
3. Create `MDA_V2_SEQUENCE_STATE.json` with current_prompt=`P00`, completed_prompts=[], accepted_tokens=[], next_prompt=`P01`, and gate_policy=`sequential_token_required`.
4. Create `MDA_V2_RUN_MANIFEST.md` documenting branch, commit, dirty status, prompt pack path, execution mode, and stop-rule model.
5. Create `MDA_V2_MASTER_TRACEABILITY_MATRIX.csv` with the required header and initial rows for P00 artifacts.
6. Create `MDA_V2_CONTROLLED_GAP_LEDGER.csv`, `MDA_V2_DECISION_LOG.md`, and `MDA_V2_REPAIR_PROMPT_QUEUE.md` with headers and no hidden gaps.
7. Verify file placement against `.ai/CONVENTIONS.md` and `.ai/AI-WORKFLOW.md` before creating any file.
8. Do not modify runtime code in P00.
...
```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P00_master_orchestrator_and_sequence_controller_HANDOFF_PACKET.md
```
...
- Prompt out of order must not execute P05 before P04 token exists.
- Missing previous token must create locked state and recovery prompt.
- Report root missing must be created in allowed location only.
- Dirty worktree must be recorded, not overwritten.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any missing governance file is P1 until located or logged as environment blocker.
- Any new file outside approved report root is P1 and must be moved.
