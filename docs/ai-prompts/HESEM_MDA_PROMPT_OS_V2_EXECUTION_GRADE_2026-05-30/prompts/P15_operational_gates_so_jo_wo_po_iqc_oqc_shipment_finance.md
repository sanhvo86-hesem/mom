# P15 — Operational release gates across SO, JO, WO, PO, IQC, OQC, shipment, maintenance, and finance
...
| --- | --- |
| Prompt ID | `P15` |
| Title | Operational release gates across SO, JO, WO, PO, IQC, OQC, shipment, maintenance, and finance |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P14_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P15_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `planning_production + procurement + quality + inventory + finance` |
| Root / scope code | `OPS-GATES` |

## Prompt objective

Connect MDA authority to every operational pathway so released master data controls real work and prevents unsafe or invalid operations.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P14_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/api/controllers/*Sales*`
- `mom/api/controllers/*WorkOrder*`
- `mom/api/controllers/*Purchase*`
- `mom/api/services/*Order*`
- `mom/api/services/*Inventory*`
- `mom/api/services/*Inspection*`
- `mom/database/migrations/*orders*`
...
1. Map existing sales order, job order, work order, purchase order, receiving, inspection, material issue, completion, OQC, shipment, invoice/payment, period close, maintenance work order, and training gates.
2. Define `ReadinessCheck` APIs for SO release, JO creation, WO release/start, PO approval, receiving/IQC, putaway, material issue, operation complete, OQC, shipment, AP/AR posting, maintenance release.
3. Define gate reason model for UI and API: blocking root, blocking record, severity, human-readable reason, re-anchor route, corrective command, required evidence.
4. Ensure every gate reads canonical authority: item/rev, UOM, customer approval, supplier approval, engineering package, machine eligibility, tool/gage/fixture readiness, employee qualification, inventory/lot status, quality hold, finance period.
5. Simulate quote-to-cash, procure-to-pay, plan-to-produce, inspection-to-release, maintenance-to-production, and quality-to-CAPA flows end-to-end.
...
```bash
grep -R "sales-orders\|job-orders\|work-orders\|purchase-orders\|receiving\|shipment\|invoice\|period\|readiness" -n mom docs .ai _reports | head -300
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P15_operational_gates_so_jo_wo_po_iqc_oqc_shipment_finance_HANDOFF_PACKET.md
```
...
- SO release blocked by customer item approval missing.
- JO creation blocked by engineering package missing.
- WO release blocked by machine/tool/gage/operator/material.
- PO blocked by supplier cert expiry.
- Shipment blocked by open NCR/quality hold.
- Invoice blocked by closed period or 3-way match.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any operational path can bypass canonical MDA gate is P1/P0.
- Any gate reason lacks re-anchor to authority record is P2 but must be logged.
