# P16 — MES runtime, edge events, machine data, OEE, offline sync, and connected-worker readiness
...
| --- | --- |
| Prompt ID | `P16` |
| Title | MES runtime, edge events, machine data, OEE, offline sync, and connected-worker readiness |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P15_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P16_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `mes_execution + connected_worker + event_integration` |
| Root / scope code | `MES-RUNTIME + MDEV` |

## Prompt objective

Design MES runtime event authority for machine/operator/tool/material execution and derive OEE/alarms/quality events without making analytics authority.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P15_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/data/mes if present`
- `mom/api/services/*Mes*`
- `mom/api/controllers/*Mes*`
- `mom/database/migrations/*mes*`
- `mom/scripts or assets related dispatch/OEE`
- `P09 machine outputs`
- `P15 gates`
...
1. Audit MES runtime stores, adapters, MTConnect/OPC UA references, machine raw events, production reports, dispatch board, connected worker routes, offline sync, OEE dashboards, alarms/playbooks.
2. Define raw event append-only authority, derived event pipeline, idempotency key, adapter heartbeat, data quality flags, stale event policy, event replay, and operator confirmation where machine data is insufficient.
3. Define execution commands/events: `StartOperation`, `PauseOperation`, `RecordMachineCycle`, `RecordScrap`, `RecordDowntime`, `RecordToolUsage`, `RecordInspectionResult`, `CompleteOperation`, `SyncOfflineExecutionPacket`.
4. Define OEE derivation boundaries: analytics can display but cannot mutate execution truth; corrections require governed command.
5. Define connected-worker UI needs: clear gate reasons, work instructions, data collection, offline draft, sync conflict, e-sign prompts, accessibility, scan-first flow.
6. Simulate duplicate events, out-of-order events, stale adapter, offline completion, operator mismatch, scrap/rework, quality signal auto-hold, and OEE correction.
...
```bash
grep -R "MTConnect\|OPC\|OEE\|machine event\|mes-runtime\|adapter\|heartbeat\|downtime\|dispatch" -n mom docs .ai _reports | head -320
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P16_mes_edge_machine_event_readiness_oee_HANDOFF_PACKET.md
```
...
- Duplicate raw MTConnect event replay.
- Out-of-order completion event.
- Offline operator completes operation then sync conflicts.
- Alarm threshold triggers quality hold.
- OEE dashboard attempts correction without command.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any analytics/dashboard becomes MES authority is P0.
- Any raw event de-duplication missing idempotency design is P1.
- Any direct machine-control command in app layer is P0.
