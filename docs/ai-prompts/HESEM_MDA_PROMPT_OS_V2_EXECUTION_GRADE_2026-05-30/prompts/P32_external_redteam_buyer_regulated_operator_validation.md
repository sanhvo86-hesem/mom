# P32 — External red-team, buyer, regulated customer, operator, maintainer, supplier, and auditor validation
...
| --- | --- |
| Prompt ID | `P32` |
| Title | External red-team, buyer, regulated customer, operator, maintainer, supplier, and auditor validation |
| Mode | `AUDIT_AND_REPAIR_ONLY` |
| Required previous token | `P31_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P32_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `redteam + product_validation` |
| Root / scope code | `REDTEAM` |

## Prompt objective

Attack the whole V2 package and any implementation waves from external viewpoints, then generate repair queue and acceptance scorecard.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P31_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `P00-P31 outputs`
- `MDA_V2_CONTROLLED_GAP_LEDGER.csv`
- `MDA_V2_MASTER_TRACEABILITY_MATRIX.csv`
- `MDA_V2_RUN_MANIFEST.md`
...
1. Score all criteria 1-5: authority clarity, no-guess discipline, domain coverage, schema soundness, command completeness, workflow/evidence/e-sign, MES operability, quality containment, inventory ledger, migration safety, security, observability, simulation depth, benchmark grounding, regulatory applicability, frontend projection safety, data governance, implementation handoff, risk realism, operator usability, buyer readiness.
2. No critical criterion may score below 4 for design acceptance; runtime acceptance remains blocked until implementation waves pass.
3. Run adversarial personas: enterprise buyer, plant manager, quality auditor, FDA/regulated auditor, CNC operator, maintenance manager, supplier quality engineer, security/OT reviewer, SRE, finance controller, data steward, AI governance lead.
4. Create `MDA_V2_EXTERNAL_REDTEAM_REPORT.md`, `MDA_V2_REPAIR_PROMPT_QUEUE.md`, `MDA_V2_SCORECARD.csv`, and append controlled gaps.
5. If score < 4 on any critical criterion, run repair synthesis and rerun score; do not emit pass until repaired or blocked with exact self-contained repair prompt.
...
```bash
ls _reports/agent-audits/mda-prompt-os-v2-2026-05-30 | sort | sed -n "1,240p"
```
```bash
grep -R "Decision token\|PASS\|BLOCKED\|REPAIR_REQUIRED" -n _reports/agent-audits/mda-prompt-os-v2-2026-05-30 | tail -120
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P32_external_redteam_buyer_regulated_operator_validation_HANDOFF_PACKET.md
```
...
- Buyer asks how MDA prevents shipment with open NCR.
- Auditor asks proof of e-sign meaning.
- Operator asks why WO release is blocked.
- Security asks how AI cannot approve.
- SRE asks how cutover rollback works.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any critical score below 4 is P1 for package acceptance.
- Any hidden P0/P1 gap discovered must reopen the relevant prior prompt or enter repair queue.
