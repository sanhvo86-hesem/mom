# P20 — JSON to PostgreSQL migration, shadow sync, reconciliation, cutover, fallback, and rollback
...
| --- | --- |
| Prompt ID | `P20` |
| Title | JSON to PostgreSQL migration, shadow sync, reconciliation, cutover, fallback, and rollback |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P19_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P20_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `platform_backend + data_migration + sre` |
| Root / scope code | `MIGRATION-CUTOVER` |

## Prompt objective

Design and gate safe migration from JSON compatibility authority/shadow-write to PostgreSQL primary/only authority without data loss.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P19_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
1. Audit DataLayer migration modes, RuntimeShadowSync, JSON stores, PG rebuilds, reconciliation scripts, backup/restore tooling, config flags, and live runtime evidence.
2. Define migration stages per root: `JSON_ONLY`, `SHADOW_WRITE`, `POSTGRES_PRIMARY`, `POSTGRES_ONLY`, with entry/exit criteria and rollback.
3. Define reconciliation runner: count, key match, field hash, lifecycle/status, references, orphan records, duplicate aliases, UOM conversion consistency, evidence links, command/audit/outbox consistency.
4. Define cutover rehearsal protocol: snapshot backup, dry-run import, shadow diff, blocker threshold zero, switch read primary, monitor fallback, restore drill, rollback decision phrases.
5. Define root-by-root migration order: UOM -> item/rev/site -> customer/supplier approvals -> engineering package -> machine/tool/gage/operator -> locations/lot/serial/holds -> operational gates.
6. Simulate JSON missing, PG duplicate, field drift, failed migration halfway, rollback after partial cutover, parallel write, and stale cache.
...
```bash
grep -R "JSON_ONLY\|SHADOW_WRITE\|POSTGRES_PRIMARY\|POSTGRES_ONLY\|RuntimeShadowSync\|reconciliation\|cutover\|rollback" -n mom docs .ai _reports | head -360
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P20_json_postgresql_migration_reconciliation_cutover_rollback_HANDOFF_PACKET.md
```
...
- JSON record absent but PG mirror exists.
- Shadow write succeeds in JSON but fails in PG.
- Cutover switch enabled with drift > 0.
- Rollback after new PG-only command writes data.
- Cache returns stale authority after cutover.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any cutover without backup and restore drill is P0.
- Any blocker drift tolerated for governed authority is P1/P0.
