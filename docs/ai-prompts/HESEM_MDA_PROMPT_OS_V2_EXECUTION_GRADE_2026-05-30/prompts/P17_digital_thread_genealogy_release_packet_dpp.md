# P17 — Digital thread, genealogy, release packet, trusted record, DPP/CoC/CoA, and recall authority
...
| --- | --- |
| Prompt ID | `P17` |
| Title | Digital thread, genealogy, release packet, trusted record, DPP/CoC/CoA, and recall authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P16_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P17_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `digital_thread + traceability_serialization + quality + customer_delivery` |
| Root / scope code | `DIGITAL-THREAD` |

## Prompt objective

Create an end-to-end operational truth graph linking customer requirements, part revision, engineering package, execution, material genealogy, inspections, evidence, shipment, and postmarket quality.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P16_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/database/migrations/*genealogy*`
- `mom/database/migrations/*release*`
- `mom/database/migrations/*shipment*`
- `P06 item`
- `P11 inventory`
- `P12 quality`
- `P16 MES`
...
1. Audit trusted release record, genealogy migrations/services, release manifest authority proof, evidence packets, shipment/compliance, and DPP/passport references.
2. Define graph node types and edge types: customer requirement, customer part approval, item revision, BOM/routing/CP/IP, SO/JO/WO, operation, machine, operator, tool, gage, lot, serial, container, inspection result, hold, NCR, MRB, CAPA, shipment, CoC/CoA/DPP.
3. Define immutable release packet and as-executed packet: planned hash, actual hash, evidence bundle, signature bundle, deviations, waivers, nonconformance links, audit chain.
4. Define APIs: `GET /api/v1/digital-thread/{root}/{id}`, `GET /api/v1/lots/{id}/genealogy`, `POST /api/v1/commands/IssueReleasePacket`, `POST /api/v1/commands/RunRecallTrace`.
5. Simulate recall by material lot, serial genealogy, supplier cert OOT, customer complaint, rework loop, lot split/merge, shipment certificate regeneration, and DPP issuance.
...
```bash
grep -R "trusted release\|release packet\|genealogy\|traceability\|DPP\|passport\|certificate\|shipment" -n mom docs .ai _reports | head -320
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P17_digital_thread_genealogy_release_packet_dpp_HANDOFF_PACKET.md
```
...
- Recall by heat number.
- Output serial missing input genealogy.
- Certificate generated after quality hold opened.
- Customer complaint traces to supplier process approval.
- Rework WO creates second genealogy path.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any certificate/DPP generated without complete authority/evidence chain is P0/P1.
- Any recall query omits shipped customers is P0.
