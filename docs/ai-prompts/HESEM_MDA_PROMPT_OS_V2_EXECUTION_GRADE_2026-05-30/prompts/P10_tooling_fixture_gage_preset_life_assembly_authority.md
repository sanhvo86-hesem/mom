# P10 — Tooling, dao cụ, fixture, gage, preset, life, crib, and assembly authority
...
| --- | --- |
| Prompt ID | `P10` |
| Title | Tooling, dao cụ, fixture, gage, preset, life, crib, and assembly authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P09_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P10_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `tooling_control + mes_execution + quality_improvement + inventory_logistics` |
| Root / scope code | `TOOL + FIXTURE + GAGE + TOOL_ASSEMBLY` |

## Prompt objective

Build the complete tooling/gage/fixture authority needed for CNC/discrete manufacturing readiness, quality, life control, and traceability.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P09_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/database/migrations/*tool*`
- `mom/database/migrations/*mes*`
- `P08 engineering package outputs`
- `P09 machine outputs`
...
1. Audit existing `tools`, `tool_transactions`, `mes_tool_assemblies`, gage/fixture stores, crib inventory, regrind/scrap/issue/return logic, calibration/MSA evidence, and routing operation tool lists.
2. Define separate objects: cutting tool, insert, holder, preset, tool assembly, fixture, gage, crib location, life counter, calibration/MSA profile, issue/return/regrind/scrap/breakage event.
3. Define commands: `RegisterTool`, `RegisterHolder`, `CreateToolAssembly`, `PresetToolAssembly`, `IssueToolToWorkOrder`, `ReturnTool`, `RecordToolUsage`, `RecordToolBreakage`, `SendToolToRegrind`, `ScrapTool`, `RegisterFixture`, `RegisterGage`, `CalibrateGage`, `ApproveGageForInspection`.
4. Define gates: tool life enough for planned quantity, gage calibration valid, fixture compatible, holder/machine compatible, preset evidence present, tool not issued elsewhere, breakage containment created, regrind count within limit.
5. Define suspect quantity containment when tool breaks during operation and tie to NCR/hold/MRB as needed.
6. Simulate concurrent tool issue, life exhaustion mid-run, gage calibration expiry, fixture mismatch, tool breakage, regrind loop, and wrong tool offset.
...
```bash
grep -R "tools\|tool_transactions\|tool_assemblies\|fixture\|gage\|calibration\|regrind\|crib" -n mom/database mom/api docs .ai | head -240
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P10_tooling_fixture_gage_preset_life_assembly_authority_HANDOFF_PACKET.md
```
...
- Two WOs issue the same tool assembly concurrently.
- Tool life remaining cannot cover planned operation quantity.
- Gage calibration expired before final inspection.
- Tool breaks after 40 pieces; suspect quantity must be held.
- Fixture approved for old revision only.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any inspection gage used without valid calibration policy is P1/P0 for regulated/high risk.
- Any breakage without suspect containment design is P1.
