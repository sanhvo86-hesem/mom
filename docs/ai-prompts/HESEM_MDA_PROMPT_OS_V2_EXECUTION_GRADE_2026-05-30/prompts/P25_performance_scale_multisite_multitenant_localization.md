# P25 — Performance, scale, multi-site, multi-tenant, localization, and vertical-pack readiness
...
| --- | --- |
| Prompt ID | `P25` |
| Title | Performance, scale, multi-site, multi-tenant, localization, and vertical-pack readiness |
| Mode | `DESIGN_AND_ARCHITECTURE_ONLY` |
| Required previous token | `P24_PASS_READY_FOR_NEXT` |
| Unlock token | `P25_PASS_READY_FOR_NEXT` |
| Domain affected | `platform_architecture + productization` |
| Root / scope code | `SCALE-PRODUCT` |

## Prompt objective

Design HESEM MDA for multi-site/global-local control, scale, vertical industry packs, localization, performance, and productization without premature infrastructure rewrite.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P24_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
1. Define multi-site/global-local master data model: enterprise global item, site item profile, customer/supplier site scope, machine/work center local, UOM global, qualifications site/process scoped, effectivity by site/customer/lot/serial.
2. Define performance budgets for command APIs, readiness checks, search, AR shell load, reconciliation, genealogy query, recall query, machine event ingestion, dashboards, report export.
3. Define caching rules: projections/cache are read models, invalidation strategy, stale read handling, re-anchor route, cache audit, no cache authority.
4. Define tenant/site security, data partitioning, localization, multilingual labels, timezone/calendar/shift, currency/tax/UOM, numbering/serialization policy.
5. Define vertical packs: CNC precision machining, aerospace/AS9100, automotive/IATF/APQP/PPAP, medical/ISO 13485, electronics/ESD/MSL, food/beverage, process/batch where applicable.
6. Simulate 10 sites, 1M lots, 10M machine events, cross-site item revision, customer-specific labeling, timezone shift handoff, and recall performance.
...
```bash
grep -R "site\|tenant\|timezone\|locale\|currency\|performance\|cache\|vertical\|AS9100\|IATF\|ISO 13485" -n mom docs _reports .ai | head -340
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P25_performance_scale_multisite_multitenant_localization_HANDOFF_PACKET.md
```
...
- Global item revision active at site A but not site B.
- Recall query across 10 sites under SLA.
- Cache says supplier approved after suspension.
- Timezone boundary affects shelf-life expiry.
- Customer label template differs by ship-to site.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any cache/projection becomes authority is P0.
- Any multi-site data leak is P0.
