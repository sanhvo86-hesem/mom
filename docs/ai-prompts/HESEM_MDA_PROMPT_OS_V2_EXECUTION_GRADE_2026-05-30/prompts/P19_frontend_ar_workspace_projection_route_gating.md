# P19 — Frontend authoritative record shells, workspace projections, route grammar, and action gating
...
| --- | --- |
| Prompt ID | `P19` |
| Title | Frontend authoritative record shells, workspace projections, route grammar, and action gating |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P18_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P19_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `frontend + ux + authority` |
| Root / scope code | `AR + WS` |

## Prompt objective

Ensure frontend cannot hide authority or mutate from workspaces; create AR shells and projection disable reasons for all MDA roots.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P18_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/design`
- `mom/contracts`
- `_reports/module-template-v4 if present`
- `P13 command outputs`
- `P15 gates`
...
1. Audit route grammar, portal shell, existing AR shells, workspaces, data-route-class/data-authority-class attributes, live API toggles, fallback behavior, disable button reasons, action rails, evidence/audit/signature tabs.
2. Define MDA AR shells under `/ops/records/{resource_family}/{record_id}` for items, item revisions, customers, suppliers, equipment, machines, tools, gages, fixtures, employees/users/qualifications, engineering packages, lots/serials, holds, NCR/CAPA.
3. Define workspaces as projections only: master data worklist, item search, supplier/customer dashboards, machine readiness board, tool crib, quality hold board, dispatch board.
4. Define UI action gating: every mutation button must call command API, show gate reason, re-anchor to AR, require stale projection refresh, and never update state locally as authority.
5. Define accessibility and operator UX requirements: scan-first, keyboard, WCAG, mobile, visible status, evidence preview, offline conflict messages.
6. Simulate stale projection, unauthorized button, offline draft, live API fallback, missing gate reason, record alias collision, browser drift, and cross-route re-anchor.
...
```bash
grep -R "data-route-class\|data-authority-class\|requires-reanchor\|/ops/records\|hmv4-live-api\|dispatch-board\|AR\|WS" -n mom docs _reports .ai | head -360
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P19_frontend_ar_workspace_projection_route_gating_HANDOFF_PACKET.md
```
...
- Workspace row tries direct status update.
- Stale projection button remains enabled after hold opened.
- Alias record ID opens wrong AR.
- Live API fails and UI silently enables mutation.
- Screen reader cannot understand blocked action reason.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any workspace mutation without AR re-anchor is P0.
- Any UI claims success without command response is P1.
