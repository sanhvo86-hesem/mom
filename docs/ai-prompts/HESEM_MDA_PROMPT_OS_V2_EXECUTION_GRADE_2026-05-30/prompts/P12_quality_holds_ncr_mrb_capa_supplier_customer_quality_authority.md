# P12 — Quality authority: holds, NCR/NQCASE, MRB, CAPA, supplier/customer quality, defect catalogs, SPC gates
...
| --- | --- |
| Prompt ID | `P12` |
| Title | Quality authority: holds, NCR/NQCASE, MRB, CAPA, supplier/customer quality, defect catalogs, SPC gates |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P11_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P12_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `quality_improvement + eQMS + supplier_quality + customer_quality` |
| Root / scope code | `HOLD + NQCASE + CAPA + INSP` |

## Prompt objective

Define canonical quality authority that blocks unsafe operational release and links inspections, holds, NCR/MRB/CAPA, supplier/customer quality, evidence, audit, and COPQ.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P11_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/database/migrations/*quality*`
- `mom/api/controllers/*Nonconformance*`
- `mom/api/services/*Capa*`
- `mom/api/services/*Quality*`
- `docs/backend/WORKFLOW_STATUS_UNIFICATION_SPEC.md`
- `P08 engineering outputs`
- `P11 inventory outputs`
...
1. Audit existing NQCASE/CAPA/insp/defect/hold/control-plan/inspection-plan/services/routes and prior slice status.
2. Define canonical hold service as the only source for blocking SO/JO/WO/PO/receipt/issue/completion/OQC/shipment/payment where quality dictates.
3. Define defect catalog, reason codes, severity, risk class, containment policy, MRB disposition, rework/scrap/use-as-is/return-to-supplier/repair, SCAR, customer deviation/waiver, CAPA trigger rules, COPQ posting.
4. Define commands: `ApplyQualityHold`, `ReleaseQualityHold`, `CreateNcr`, `OpenMrb`, `DispositionMrb`, `CreateCapa`, `ApproveCapaPlan`, `VerifyCapaEffectiveness`, `CloseCapa`, `OpenScar`, `RecordCustomerComplaint`, `RecordOqcFailure`.
5. Define e-sign/audit requirements for disposition, use-as-is, override, CAPA closure, hold release, and customer waiver.
6. Simulate OQC fail, incoming inspection fail, supplier cert missing, SPC out-of-control, tool breakage suspect hold, OOT calibration impact, CAPA overdue, and MRB scrap cost.
...
```bash
grep -R "nonconformance\|NQCASE\|CAPA\|hold\|MRB\|defect\|inspection\|quality" -n mom/database mom/api docs .ai _reports | head -280
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P12_quality_holds_ncr_mrb_capa_supplier_customer_quality_authority_HANDOFF_PACKET.md
```
...
- OQC failure creates hold and NCR; shipment blocked.
- MRB scrap posts COPQ and inventory decrement.
- Use-as-is requires customer approval for customer-controlled part.
- CAPA close without effectiveness rejected.
- Quality hold release by unauthorized user rejected.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any quality hold bypass in shipment/issue/completion is P0.
- Any disposition without audit/evidence/e-sign where required is P0/P1.
