# P13 — Domain command bus, idempotency, OpenAPI contracts, problem details, and outbox/event authority
...
| --- | --- |
| Prompt ID | `P13` |
| Title | Domain command bus, idempotency, OpenAPI contracts, problem details, and outbox/event authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P12_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P13_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `platform_backend + integration` |
| Root / scope code | `COMMAND + API + EVENT` |

## Prompt objective

Convert governed master-data and operational mutations into command-only APIs with idempotency, transactionality, audit/evidence/outbox, problem details, and testable OpenAPI contracts.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P12_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
1. Audit existing command spec, routing, idempotency, outbox/events, OpenAPI, problem details, CSRF/session/MFA/re-auth patterns, and Generic CRUD denylist.
2. Define command envelope: `POST /api/v1/commands/{CommandName}`, `Idempotency-Key`, `X-Correlation-Id`, CSRF for browser, re-auth/e-sign where required, transaction boundary, audit, evidence, outbox, retry semantics.
3. Define problem details format with code, message, correlation_id, command, retryable, violations, source pointer, gate reason, re-anchor path, and operator-safe wording.
4. Produce command coverage matrix for every root and every governed mutation from P03-P12.
5. Define implementation plan for command router/handlers without rewriting existing MVC architecture.
6. Simulate retries, duplicate idempotency key, concurrent release, stale UI projection, partial DB write, outbox failure, e-sign failure, and rollback.
...
```bash
grep -R "Idempotency-Key\|X-Correlation-Id\|Problem Details\|RFC 9457\|outbox\|domain_command_required\|GenericCrud" -n mom docs .ai _reports | head -260
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P13_command_bus_idempotency_problem_details_contracts_HANDOFF_PACKET.md
```
...
- Retry ReleaseItemRevision after network timeout.
- Two users approve same supplier process concurrently.
- Outbox insert fails after state update attempt.
- Stale workspace tries to mutate.
- Generic CRUD updates governed table.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any governed mutation lacks command owner is P1.
- Any command writes state without audit/evidence/outbox transaction discipline is P0/P1.
