# P24 — Validation, regulated eQMS package, traceability matrix, Part 11/Annex 11/GAMP applicability
...
| --- | --- |
| Prompt ID | `P24` |
| Title | Validation, regulated eQMS package, traceability matrix, Part 11/Annex 11/GAMP applicability |
| Mode | `DESIGN_AND_GOVERNANCE_ONLY` |
| Required previous token | `P23_PASS_READY_FOR_NEXT` |
| Unlock token | `P24_PASS_READY_FOR_NEXT` |
| Domain affected | `validation + eQMS + regulatory` |
| Root / scope code | `VALIDATION` |

## Prompt objective

Define validation package for regulated or high-assurance deployments without overclaiming validated production status.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P23_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
1. Define intended use, risk class, GxP applicability, validation boundary, configuration vs customization, electronic record/signature boundary, supplier/customer/regulatory applicability per vertical.
2. Produce requirements traceability matrix linking user needs, functional requirements, technical requirements, controls, tests, evidence, risk mitigations, and release artifacts.
3. Define validation artifacts: URS, FS, DS, risk assessment, test protocol, test report, deviation log, trace matrix, data integrity assessment, audit/e-sign assessment, release readiness report, rollback rehearsal record.
4. Define no-overclaim wording: design package, prototype, pre-production readiness, not validated production system unless formal package completed and approved.
5. Define periodic review, change control, revalidation triggers, incident/CAPA links, supplier audit/AI model review if applicable.
6. Simulate audit challenge, missing test evidence, changed workflow after validation, emergency fix, AI advisory in regulated decision path, and e-sign audit trail review.
...
```bash
grep -R "Part 11\|Annex 11\|GAMP\|validation\|traceability matrix\|e-sign\|data integrity" -n docs mom _reports .ai | head -300
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P24_validation_regulated_qms_package_HANDOFF_PACKET.md
```
...
- Customer auditor asks proof that AI cannot approve CAPA.
- Workflow changed after validation package.
- E-sign meaning missing in historical audit.
- Test protocol exists but no test report.
- Emergency fix has no rollback rehearsal.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any prompt claims validated production without formal package is P0/P1.
- Any regulated electronic signature lacks audit/meaning is P0.
