# P30 — Implementation Wave 3 Codex prompt: canonical hold service, inventory gates, genealogy, shipment block
...
| --- | --- |
| Prompt ID | `P30` |
| Title | Implementation Wave 3 Codex prompt: canonical hold service, inventory gates, genealogy, shipment block |
| Mode | `CODE_MUTATION_ALLOWED_AFTER_BRANCH` |
| Required previous token | `P29_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P30_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `quality + inventory + traceability + shipment` |
| Root / scope code | `WAVE3` |

## Prompt objective

Implement canonical quality hold and inventory traceability gates that block unsafe issue/completion/OQC/shipment and produce audit evidence.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P29_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
1. Create branch `codex/mda-v2-wave3-quality-inventory-traceability-20260530`.
2. Implement or wire canonical `QualityHoldService` as the source read by material issue, completion, OQC, shipment, and relevant dashboards.
3. Add focused gates: material issue blocked by lot hold/shelf-life/cert missing; completion blocked by missing genealogy; shipment blocked by open hold/OQC fail; MRB disposition updates inventory/COPQ through controlled ledger where available.
4. Implement minimal recall/genealogy query if existing tables support it; otherwise create fail-safe API contract + tests documenting missing physical chain.
5. Add focused tests for hold apply/release, issue block, shipment block, genealogy missing, idempotent hold command, and audit event creation.
6. Do not fake inventory/cost postings. If finance ledger is not available, record controlled gap and ensure operational block still works.
...
```bash
grep -R "QualityHold\|hold\|shipment\|issue\|inventory\|genealogy\|OQC\|MRB\|COPQ" -n mom/api mom/database mom/tests docs .ai | head -380
```
```bash
php -l <touched_php_files>
```
```bash
composer --working-dir=mom run test -- --filter "Hold|Inventory|Shipment|Genealogy|Quality" || true
```
```bash
...
```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P30_implementation_wave3_quality_inventory_traceability_codex_prompt_HANDOFF_PACKET.md
```
...
- Shipment blocked by open hold.
- Issue blocked by expired lot.
- MRB scrap decreases inventory or logs finance controlled gap.
- Recall query by heat number.
- Duplicate hold command idempotent.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any shipment with active quality hold allowed is P0.
- Any inventory mutation without ledger/audit plan is P0/P1.
