# P23 — Scenario DSL, automation harness, simulation library, regression matrix
...
| --- | --- |
| Prompt ID | `P23` |
| Title | Scenario DSL, automation harness, simulation library, regression matrix |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P22_PASS_READY_FOR_NEXT` |
| Unlock token | `P23_PASS_READY_FOR_NEXT` |
| Domain affected | `qa_automation + simulation` |
| Root / scope code | `SCENARIO-DSL` |

## Prompt objective

Turn operational simulations into executable regression assets covering positive, negative, race, idempotency, offline, security, regulatory, rollback, performance, and cross-module paths.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P22_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
1. Design a scenario DSL format using JSON/YAML/CSV suitable for repo tools, with preconditions, commands/APIs, fixtures, expected state, forbidden state, assertions, cleanup, and evidence capture.
2. Create a scenario catalog of at least 200 scenario IDs across P05-P22 domains; mark P0/P1 critical scenarios.
3. Define runner architecture that can execute safe focused tests now and expand later without requiring live production systems.
4. Map scenarios to PHPUnit, PHPStan/static grep, Playwright/E2E, migration dry-run, data reconciliation, API contract tests, and manual validation where automation is not yet available.
5. Define report outputs: pass/fail, artifact paths, screenshots/logs, DB diffs, audit events, evidence packets, correlation IDs, coverage gaps.
6. Simulate the simulation harness itself: duplicate scenario IDs, flaky browser, missing fixture, race non-determinism, environment tool missing, false positive pass.
...
```bash
find mom/tests tools/scripts _reports -maxdepth 4 -type f | sed -n "1,200p"
```
```bash
grep -R "scenario\|fixture\|Playwright\|runtime_assurance\|phpunit\|idempotency" -n mom tests tools docs _reports .ai | head -360
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P23_scenario_dsl_automation_harness_HANDOFF_PACKET.md
```
...
- Harness passes while no assertion ran.
- Duplicate scenario ID hides coverage.
- Browser unavailable.
- Race test flaky.
- Fixture missing creates false success.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any P0/P1 gate lacks at least one scenario is P1.
- Any automation pass without assertions is P0 for QA credibility.
