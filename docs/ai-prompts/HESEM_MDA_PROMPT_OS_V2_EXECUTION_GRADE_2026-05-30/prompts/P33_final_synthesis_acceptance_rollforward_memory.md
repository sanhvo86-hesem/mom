# P33 — Final synthesis, acceptance decision, roll-forward package, and project memory update
...
| --- | --- |
| Prompt ID | `P33` |
| Title | Final synthesis, acceptance decision, roll-forward package, and project memory update |
| Mode | `SYNTHESIS_AND_HANDOFF_ONLY` |
| Required previous token | `P32_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P33_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `portfolio_governance + project_memory` |
| Root / scope code | `FINAL` |

## Prompt objective

Produce final package summary, acceptance decision, no-overclaim statement, next implementation prompts, and project memory handoff for future chats.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P32_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `P00-P32 outputs`
- `MDA_V2_EXTERNAL_REDTEAM_REPORT.md`
- `MDA_V2_CONTROLLED_GAP_LEDGER.csv`
- `MDA_V2_DECISION_LOG.md`
- `README.md`
...
1. Synthesize P00-P32 into `MDA_V2_FINAL_MASTER_BUILD_PLAN.md`, `MDA_V2_FINAL_ACCEPTANCE_DECISION.md`, `MDA_V2_IMPLEMENTATION_HANDOFF_PROMPTS.md`, `MDA_V2_NEXT_CHAT_MEMORY.md`, and final manifest.
2. State clearly: design/governance/planning acceptance vs runtime implementation status; do not claim production-ready or validated unless evidence exists.
3. List exact remaining controlled gaps with severity, owner, acceptance criteria, next prompt, and blocking effect.
4. List exact implementation wave prompts and branch strategy for Codex/Claude agents.
5. Create a short memory file the user can put into GPT Project source when context grows too large, including current decisions, run root, key gaps, next action, and no-guess rules.
6. Simulate future context loss: next AI starts with only memory file and must know what to do next.
...
```bash
ls _reports/agent-audits/mda-prompt-os-v2-2026-05-30 | sort | tail -120
```
```bash
grep -R "P[0-9][0-9]_PASS\|BLOCKED\|REPAIR_REQUIRED" -n _reports/agent-audits/mda-prompt-os-v2-2026-05-30 | tail -200
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P33_final_synthesis_acceptance_rollforward_memory_HANDOFF_PACKET.md
```
...
- New chat loses context.
- Implementation agent starts wrong wave.
- User asks production readiness.
- P2 gaps hidden by final summary.
- Repo changed after package.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any final package hides remaining gaps is P0/P1.
- Any final acceptance claims runtime-complete without implementation evidence is P1.
