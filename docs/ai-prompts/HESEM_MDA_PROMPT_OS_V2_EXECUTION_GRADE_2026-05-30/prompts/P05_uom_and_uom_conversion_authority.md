# P05 — UOM and UOM conversion runtime authority
...
| --- | --- |
| Prompt ID | `P05` |
| Title | UOM and UOM conversion runtime authority |
| Mode | `DESIGN_THEN_IMPLEMENTATION_PROMPT_BLUEPRINT` |
| Required previous token | `P04_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P05_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `master_data + inventory_logistics + finance` |
| Root / scope code | `UOM` |

## Prompt objective

Resolve UOM as a first-class runtime authority dependency for item, BOM, purchasing, inventory, inspection, costing, packaging, shipment, and machine/material consumption.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P04_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
1. Audit existing `uom` and `mdm_uom_conversions` schemas, services, registry maps, runtime loading, and reconciliation.
2. Design canonical UOM lifecycle: draft/active/retired, dimension class, base unit, conversion source, precision, rounding, tolerance, effective date, site/customer/supplier scope, evidence, approval, audit.
3. Define commands: `CreateUom`, `ActivateUom`, `RetireUom`, `CreateUomConversion`, `ApproveUomConversion`, `RetireUomConversion`, `RunUomReconciliation`.
4. Define gates: item create requires base UOM; BOM component requires compatible UOM; receipt/issue/conversion requires active conversion; costing uses deterministic rounding; shipment packaging UOM cannot use draft conversion.
5. Produce exact implementation prompt for later code if runtime services are missing; do not collide with other AI UOM branches.
6. Simulate dimensional mismatch, rounding drift, duplicate conversion, expired conversion, backdated conversion, and cost variance.
...
```bash
grep -R "mdm_uom_conversions\|CREATE TABLE IF NOT EXISTS uom\|base_uom_code\|uom_code" -n mom docs .ai _reports | head -200
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P05_uom_and_uom_conversion_authority_HANDOFF_PACKET.md
```
...
- Item created with missing base UOM.
- BOM uses KG component for EA parent without conversion.
- PO receives LB while inventory is KG with stale conversion.
- Cost rollup changes after conversion precision change.
- Two agents create inverse duplicate conversion concurrently.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any item/revision prompt depends on UOM but UOM remains unclassified is P1.
- Any conversion used without precision/rounding/effectivity is P1.
