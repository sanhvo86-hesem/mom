# P06 — Item, material, part number, revision, site, variant, specification, and traceability policy authority
...
| --- | --- |
| Prompt ID | `P06` |
| Title | Item, material, part number, revision, site, variant, specification, and traceability policy authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P05_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P06_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `master_data + engineering_authority + inventory_logistics` |
| Root / scope code | `ITEM + IREV` |

## Prompt objective

Define and implementation-plan the world-class item/part/revision backbone, using existing canonical item schema and legacy field compatibility without hidden authority.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P05_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/contracts/objects/inventory_logistics--inventory-items/contract.json if present`
...
1. Anchor to existing `item`, `item_revision`, `item_site`, `item_attr`, `item_spec`, `lot_policy`, `serial_policy`, `shelf_life_policy` where physically present.
2. Map all legacy `items` and `item_revisions` fields into target profiles: identity, revision/effectivity, engineering, UOM, physical, planning, procurement, manufacturing, inventory, traceability, quality, compliance, finance/cost, packaging, customer/supplier cross-reference.
3. Separate item enterprise identity from revision technical meaning, site-local operational profile, drawing/document evidence, customer part mapping, supplier/manufacturer part mapping, substitute/alternate rules, and transaction snapshots.
4. Define commands and DTOs: `CreateItem`, `CreateItemRevision`, `SubmitItemRevision`, `ApproveItemRevision`, `ReleaseItemRevision`, `SupersedeItemRevision`, `ObsoleteItem`, `ConfigureItemSite`, `AddItemSpec`, `AddCustomerCrossRef`, `AddSupplierCrossRef`, `ApproveSubstitution`, `ChangeTraceabilityPolicy`.
5. Define immutable release snapshot: part number, revision, drawing/doc refs, material/spec bounds, BOM/routing/control plan/inspection plan package hash, lot/serial/shelf-life policy, customer/supplier approvals, evidence and signatures.
6. Produce exhaustive field catalog for material/item/part number/revision with required/optional/regulated/site-specific/customer-specific flags.
7. Simulate quote, SO, JO, WO, PO, receiving, issue, inspection, OQC, shipment, recall, ECO, supersede, substitute, obsolete, and rework scenarios.
...
```
```bash
grep -R "customer_part_number\|manufacturer_part_number\|substitute_item_id\|lot_tracking\|serial_tracking\|revision_tracking" -n mom/database mom/api docs .ai | head -200
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P06_item_material_part_revision_spec_authority_HANDOFF_PACKET.md
```
...
- Release item revision with missing UOM conversion.
- ECO changes material grade while WO is running.
- Customer revision differs from internal revision.
- Obsolete item with open inventory and SO.
- Substitute item used without customer/site approval.
- Recall by heat number must find all shipped serials.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any released revision mutable by generic update is P0.
- Any customer/supplier part mapping overwrites transaction history is P1.
- Any field catalog treats site-local planning as enterprise item identity is P1.
