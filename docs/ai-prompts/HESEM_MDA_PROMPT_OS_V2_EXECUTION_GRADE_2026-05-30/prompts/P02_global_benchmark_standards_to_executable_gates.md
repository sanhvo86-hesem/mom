# P02 — Global benchmark, standards, and executable-gate extraction
...
| --- | --- |
| Prompt ID | `P02` |
| Title | Global benchmark, standards, and executable-gate extraction |
| Mode | `RESEARCH_AND_GOVERNANCE_ONLY` |
| Required previous token | `P01_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P02_PASS_READY_FOR_NEXT` |
| Domain affected | `benchmark + standards + architecture` |
| Root / scope code | `MDA-BENCH` |

## Prompt objective

Convert world-class ERP/MOM/MES/eQMS vendor patterns and standards into explicit HESEM gates, artifacts, evidence, tests, and stop rules.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P01_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `references/RESEARCH_AND_EVIDENCE_REFERENCES.md`
- `docs/benchmark/ if present`
...
1. Use only official or primary sources for benchmark refresh where internet is available; otherwise use the supplied reference file and mark freshness as `NOT_REFRESHED`.
2. Extract patterns from SAP Digital Manufacturing, Siemens Opcenter, Rockwell Plex, MasterControl, ETQ/MasterControl-like QMS, MTConnect, OPC UA, NIST AI RMF, ISA-95/IEC 62264, ISA-88, OWASP, GAMP, Part 11/Annex 11 if applicable.
3. Do not claim HESEM is better than vendors. Extract patterns, emulate/avoid/differentiate opportunities, and map them to executable gates.
4. Create `benchmark_pattern -> HESEM artifact -> owner -> gate -> evidence -> test -> rollback -> stop_rule` matrix.
5. Define applicability gates for regulated standards: do not force Part 11/Annex 11/GAMP unless intended use and customer vertical require them, but design evidence hooks now.
6. Map SAP-style resource orchestration to WO readiness; Siemens-style as-planned/as-is to digital thread; Plex-style OT/IT and edge-to-cloud to event/readiness architecture; MasterControl-style regulated manufacturing to evidence/eDHR/eBR/e-logbook/calibration gates.
...
```bash
grep -R "ISA-95\|S95\|S88\|Part 11\|GAMP\|MTConnect\|OPC" -n docs mom .ai _reports | head -100
```
```bash
grep -R "benchmark\|SAP\|Siemens\|Plex\|MasterControl" -n docs _reports mom/data/registry | head -100
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P02_global_benchmark_standards_to_executable_gates_HANDOFF_PACKET.md
```
...
- Benchmark demands worker skills; verify HESEM translates to qualification gate, not brochure text.
- OPC/MTConnect data arrives duplicated; verify idempotent raw event contract is required.
- Regulated customer asks eDHR/eBR; verify evidence/e-sign applicability is explicit.
- AI recommends supplier approval; verify AI remains advisory.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any vendor claim without source or freshness status is P2 at minimum.
- Any standard named without artifact/gate/evidence/test mapping is P1 for this prompt.
