# P09 — Equipment, machine, work center, connectivity, maintenance, and calibration authority
...
| --- | --- |
| Prompt ID | `P09` |
| Title | Equipment, machine, work center, connectivity, maintenance, and calibration authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P08_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P09_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `maintenance_ehs + mes_execution + master_data` |
| Root / scope code | `EQP + WORK_CENTER + MACHINE + ADAPTER` |

## Prompt objective

Define machine/equipment/work-center authority and production eligibility gates for scheduling, release, execution, maintenance, calibration, safety, and connectivity.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P08_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/database/migrations/*maintenance*`
...
1. Audit existing work centers, equipment, machine store entries, MES connectivity adapters, alarms, maintenance, calibration, PM, and condition monitoring data.
2. Define machine profile: identity, site/area/line/work center, asset hierarchy, capability envelope, controller/firmware/postprocessor, scheduling/capacity, status, maintenance, PM, calibration, condition monitoring, connectivity, heartbeat, safety, qualified operators, evidence.
3. Define commands: `RegisterWorkCenter`, `RegisterMachine`, `SetMachineCapability`, `RegisterMachineAdapter`, `RecordHeartbeat`, `PutMachineOnHold`, `ReleaseMachineForProduction`, `RecordMaintenance`, `RecordCalibration`, `OpenMachineOOTInvestigation`.
4. Define one canonical `MachineProductionEligibilityService` consumed by WO release/start, dispatch, scheduling, OEE, quality signal escalation, and UI disable reasons.
5. Define gates for PM overdue, calibration expired, quality/safety hold, adapter offline, capability mismatch, required data collection offline, maintenance lockout, and operator qualification mismatch.
6. Simulate heartbeat timeout, duplicate raw events, PM due during running WO, out-of-tolerance calibration impact, machine alternative selection, and downtime alarm escalation.
...
```bash
grep -R "work_centers\|equipment\|machine\|connectivity_adapters\|alarm_catalog\|calibration\|maintenance" -n mom/database mom/api docs .ai | head -240
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P09_equipment_machine_workcenter_connectivity_maintenance_calibration_authority_HANDOFF_PACKET.md
```
...
- WO release on machine with expired calibration.
- Machine adapter heartbeat stale but operation requires machine data.
- Machine PM becomes overdue during long WO.
- Machine capability cannot meet item tolerance.
- Quality alarm auto-holds suspect quantity.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any machine status from dashboard becomes authority is P0.
- Any WO release bypasses machine eligibility service is P1.
