# P11 — Warehouse, location, lot, serial, container, inventory ledger, and traceability authority
...
| --- | --- |
| Prompt ID | `P11` |
| Title | Warehouse, location, lot, serial, container, inventory ledger, and traceability authority |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P10_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P11_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `inventory_logistics + traceability_serialization + finance` |
| Root / scope code | `LOCATION + LOT + SERIAL + CONTAINER` |

## Prompt objective

Define authoritative inventory and traceability foundation for receipts, quarantine, issue, WIP, completion, shipment, recall, cost, and genealogy.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P10_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `mom/database/migrations/*inventory*`
- `mom/database/migrations/*warehouse*`
- `mom/database/migrations/*trace*`
- `mom/database/migrations/*finance*`
- `P06 item outputs`
- `P10 tooling outputs`
...
1. Audit warehouse locations, inventory balances, material lots, serial numbers, containers, holds, transactions, ledger, and cost hooks.
2. Define warehouse/location hierarchy: site, warehouse, zone, bin, crib, quarantine, MRB, WIP, shipping, customer/supplier-owned, ESD/temperature/cleanroom controls.
3. Define lot/serial/container authority: item revision, supplier lot, heat number, material cert, shelf life, status, genealogy policy, hold policy, split/merge, transfer, issue, consume, produce, ship, recall.
4. Define commands: `CreateLocation`, `ReceiveLot`, `AttachMaterialCertificate`, `QuarantineLot`, `AcceptLot`, `RejectLot`, `SplitLot`, `MergeLot`, `IssueMaterialToWO`, `ConsumeMaterial`, `CompleteOutputLotSerial`, `PackContainer`, `ShipContainer`, `RunRecallTrace`.
5. Define ledger invariants: append-only transactions, no negative unless policy allows, period lock, idempotency, cost reconciliation, stock status gates, FEFO/FIFO, shelf-life hold, quality hold inheritance.
6. Simulate lot split/merge, expired material, cert missing, serial duplicate, issue to wrong revision, shipment with open hold, recall by heat, period close, and cost variance.
...
```bash
grep -R "warehouse\|location\|lot\|serial\|container\|inventory\|ledger\|stock\|shipment" -n mom/database mom/api docs .ai | head -260
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P11_warehouse_lot_serial_container_traceability_inventory_ledger_HANDOFF_PACKET.md
```
...
- Split held lot; child lots inherit hold.
- Merge different item revisions; reject.
- Issue lot with expired shelf life.
- Complete output serial without full input genealogy.
- Recall heat number returns receipts, WOs, shipments, customers.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any inventory mutation without append-only ledger/audit is P0.
- Any shipment allowed with open quality hold is P0/P1.
