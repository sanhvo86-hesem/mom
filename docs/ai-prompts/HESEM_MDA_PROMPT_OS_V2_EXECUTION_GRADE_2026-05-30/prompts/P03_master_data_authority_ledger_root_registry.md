# P03 — Master Data Authority Ledger, root registry, and operational truth graph v2
...
| --- | --- |
| Prompt ID | `P03` |
| Title | Master Data Authority Ledger, root registry, and operational truth graph v2 |
| Mode | `DESIGN_AND_LEDGER_ONLY` |
| Required previous token | `P02_PASS_READY_FOR_NEXT` |
| Unlock token | `P03_PASS_READY_FOR_NEXT` |
| Domain affected | `master_data + digital_thread` |
| Root / scope code | `MDA-LEDGER` |

## Prompt objective

Define the authoritative roots, canonical keys, lifecycle owners, table/store authority, commands, workflows, downstream gates, evidence, audit, e-sign, events, and reconciliation rules for all MDA objects.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P02_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
1. Build the `MDA_V2_ROOT_AUTHORITY_LEDGER.csv` covering ITEM, IREV, CUST, SUP, EQP, MDEV, UOM, WORK_CENTER, MACHINE, TOOL, HOLDER, TOOL_ASSEMBLY, FIXTURE, GAGE, EMPLOYEE, USER, QUALIFICATION, BOM, ROUTING, OPERATION, CONTROL_PLAN, INSPECTION_PLAN, TRAVELER_TEMPLATE, NC_PROGRAM, WAREHOUSE_LOCATION, LOT, SERIAL, CONTAINER, HOLD, NCR/NQCASE, CAPA, CUSTOMER_ITEM_APPROVAL, SUPPLIER_PROCESS_APPROVAL.
2. For each root, state canonical key, alias strategy, lifecycle state, owning domain, authoritative table/service, current runtime mode, target runtime mode, mutation commands, read APIs, events, evidence, audit, e-sign meaning, stop rules, and downstream consumers.
3. Create operational truth graph adjacency: customer requirement -> item revision -> engineering package -> SO/JO/WO -> resources/materials -> execution -> inspection -> output lot/serial -> shipment -> NCR/CAPA/COPQ if any.
4. Separate identity roots from contained children, reference masters, transactional records, projections, and derived analytics.
5. Mark all non-evidenced physical objects as target additions, not existing assets.
...
```bash
grep -R "root-code\|root_code\|ITEM\|CUST\|SUP\|EQP\|MDEV" -n mom/data docs _reports .ai | head -120
```
```bash
grep -R "CREATE TABLE IF NOT EXISTS" -n mom/database/migrations | grep -E "item|customer|vendor|supplier|equipment|tool|uom|routing|bill|control|inspection|lot|serial|hold" | head -200
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P03_master_data_authority_ledger_root_registry_HANDOFF_PACKET.md
```
...
- A dashboard tries to become authority for machine status.
- A workspace writes supplier approval without AR re-anchor.
- A lot inherits item rev incorrectly after item rev supersede.
- Customer part number overwrites internal canonical item code.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any root lacks owner/key/lifecycle/authority classification is P1.
- Any projection classified as mutation authority is P0.
