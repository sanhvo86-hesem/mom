# P21 — Data quality, duplicate detection, stewardship workflow, MDM operating model
...
| --- | --- |
| Prompt ID | `P21` |
| Title | Data quality, duplicate detection, stewardship workflow, MDM operating model |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P20_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P21_PASS_READY_FOR_NEXT` |
| Domain affected | `data_governance + master_data` |
| Root / scope code | `DATA-QUALITY` |

## Prompt objective

Define data quality and stewardship system so master data remains clean, governed, deduplicated, accountable, and continuously reconciled.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P20_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
- `P03 ledger`
- `P20 reconciliation outputs`
- `mom/api/services/MasterDataService.php`
- `mom/database/migrations/*master*`
- `mom/data/registry`
...
1. Define data quality dimensions: completeness, validity, uniqueness, consistency, timeliness, lineage, ownership, authorization, referential integrity, release readiness, audit completeness.
2. Define duplicate candidate models for item aliases, customer/supplier names/sites, employee-user binding, tool/gage IDs, machine serials, UOM conversions, warehouse locations.
3. Define stewardship workflows: propose merge, review, approve, reject, split, alias, retire, change owner, periodic review, mass import review, impact analysis.
4. Define data quality scorecards and SLAs by root/domain/owner, with control tower hooks and P0/P1/P2 defect classification.
5. Define import/bulk edit rules: no direct authority mutation; staged import -> validation -> preview -> approval -> command execution -> audit/reconcile.
6. Simulate duplicate part alias, supplier duplicate legal name, employee duplicate user, tool ID collision, bulk import with bad UOM, and merge rollback.
...
```bash
grep -R "duplicate\|data quality\|steward\|alias\|external_reference\|import" -n mom docs .ai _reports | head -280
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P21_data_quality_duplicate_stewardship_mdm_operating_model_HANDOFF_PACKET.md
```
...
- Duplicate part numbers differ only by spaces/case.
- Supplier legal entity duplicated across two sites.
- Bulk import tries to activate item without approval.
- Merge two customers with open SOs.
- Retire alias used by historic transactions.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any merge alters historic transaction meaning is P0.
- Any bulk import bypasses command/evidence/audit is P1/P0.
