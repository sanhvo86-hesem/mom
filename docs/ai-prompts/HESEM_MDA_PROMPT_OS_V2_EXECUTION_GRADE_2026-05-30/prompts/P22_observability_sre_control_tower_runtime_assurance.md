# P22 — Observability, SRE, data-quality control tower, runtime assurance, and runbooks
...
| --- | --- |
| Prompt ID | `P22` |
| Title | Observability, SRE, data-quality control tower, runtime assurance, and runbooks |
| Mode | `DESIGN_AND_IMPLEMENTATION_BLUEPRINT` |
| Required previous token | `P21_PASS_READY_FOR_NEXT` |
| Unlock token | `P22_PASS_READY_FOR_NEXT` |
| Domain affected | `sre + observability + analytics` |
| Root / scope code | `OBSERVABILITY` |

## Prompt objective

Define telemetry, dashboards, alerts, runtime assurance checks, and runbooks for MDA authority, gates, commands, reconciliation, and operational safety.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P21_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
- `mom/tests/runtime_assurance_suite.php if present`
- `mom/tools`
- `_reports/runtime-assurance`
- `_reports/observability`
- `docs/release`
- `P13 command outputs`
- `P20 migration outputs`
...
1. Audit existing runtime assurance suites, telemetry, logs, dashboards, reports, release manifests, endpoint catalogs, CI checks, and data registry outputs.
2. Define metrics: command success/failure, idempotency replay, gate blocks by root, generic CRUD denies, reconciliation drift, stale projection, workflow stuck, e-sign failures, outbox lag, machine adapter heartbeat, data quality score, P0/P1 incidents.
3. Define logs/traces: correlation_id, command_id, record_id, root_code, gate_reason, user_id, role, site, idempotency_key, event_id, audit_event_id, evidence_id.
4. Define control tower dashboards for master data authority, operational gate health, migration drift, quality holds, resource readiness, security/SoD, AI advisory boundary, and release readiness.
5. Define runbooks: command failure, PG drift, cutover rollback, stale projection, machine adapter down, e-sign failure, quality hold emergency, data import rollback.
6. Simulate alert flood, silent drift, missing logs, outbox backlog, broken dashboard query, and false positive readiness.
...
```bash
grep -R "runtime_assurance\|OpenTelemetry\|telemetry\|correlation_id\|dashboard\|alert\|runbook\|outbox" -n mom docs _reports .ai | head -340
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P22_observability_sre_control_tower_runtime_assurance_HANDOFF_PACKET.md
```
...
- Generic CRUD deny spikes after release.
- Reconciliation drift silently increases.
- Outbox lag grows after command release.
- Machine heartbeat false positives.
- Dashboard shows ready while hold service blocks.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any runtime gate lacks telemetry is P2 but must be tracked.
- Any P0/P1 incident lacks runbook is P1.
