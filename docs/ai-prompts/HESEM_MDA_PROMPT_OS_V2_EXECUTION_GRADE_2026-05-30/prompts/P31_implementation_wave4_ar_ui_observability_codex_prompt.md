# P31 — Implementation Wave 4 Codex prompt: AR shells, projection gating, gate reasons, observability hooks
...
| --- | --- |
| Prompt ID | `P31` |
| Title | Implementation Wave 4 Codex prompt: AR shells, projection gating, gate reasons, observability hooks |
| Mode | `CODE_MUTATION_ALLOWED_AFTER_BRANCH` |
| Required previous token | `P30_PASS_WITH_CONTROLLED_GAPS` |
| Unlock token | `P31_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `frontend + observability + platform_backend` |
| Root / scope code | `WAVE4` |

## Prompt objective

Implement or strengthen authoritative record shell routing, workspace projection gating, gate reasons, and telemetry for MDA readiness without introducing hidden UI authority.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P30_PASS_WITH_CONTROLLED_GAPS` is present unless this is `P00`.
...
1. Create branch `codex/mda-v2-wave4-ar-ui-observability-20260530`.
2. Implement minimum AR shell routes or metadata for items, suppliers, machines, tools/gages, lots/serials, holds, and engineering packages if existing route architecture supports it.
3. Ensure workspaces show projection status and cannot mutate without command API and AR re-anchor.
4. Add gate reason rendering with blocking record, root code, severity, corrective link, and stale projection refresh.
5. Add telemetry/logging hooks for command failure, gate block, stale projection action, Generic CRUD deny, and reconciliation drift if existing telemetry patterns exist.
6. Run focused UI/contract/syntax tests; if Chromium/browser is unavailable, record blocker and run non-browser DOM/static checks.
...
```bash
grep -R "data-route-class\|data-authority-class\|/ops/records\|gate reason\|projection\|reanchor\|telemetry" -n mom docs _reports .ai | head -380
```
```bash
node --check <touched_js_files> || true
```
```bash
php -l <touched_php_files> || true
```
```bash
composer --working-dir=mom run test -- --filter "Route|Projection|Gate|Authority" || true
```
```bash
npx playwright test --grep "MDA|authority|projection" || true
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P31_implementation_wave4_ar_ui_observability_codex_prompt_HANDOFF_PACKET.md
```
...
- Workspace mutation button disabled with correct reason.
- Stale projection tries command and receives re-anchor requirement.
- AR evidence/audit/signature tabs visible.
- Telemetry emitted for gate block.
- Browser unavailable does not produce false pass.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any UI direct mutation path remains enabled is P0/P1.
- Any browser failure claimed as pass is P1.
