# P27 — Implementation Wave 0 Codex prompt: authority freeze, Generic CRUD governance, denylist, baseline repair
...
| --- | --- |
| Prompt ID | `P27` |
| Title | Implementation Wave 0 Codex prompt: authority freeze, Generic CRUD governance, denylist, baseline repair |
| Mode | `CODE_MUTATION_ALLOWED_AFTER_BRANCH` |
| Required previous token | `P26_PASS_READY_FOR_NEXT` |
| Unlock token | `P27_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `platform_backend + governance` |
| Root / scope code | `WAVE0` |

## Prompt objective

Provide and execute the first safe implementation wave: freeze governed mutation surfaces, publish denylist, add tests, preserve MVC/fallback patterns, and avoid new functional slices.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P26_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
- `mom/api/services/GenericCrudService.php if present`
...
1. Create branch `codex/mda-v2-wave0-authority-freeze-20260530` from current HEAD unless already on an isolated codex branch for this exact task.
2. Inspect `git status`, branch, remote branches, migration numbers, changed files, and AGENTS rules before edits.
3. Edit only files required for authority freeze: Generic CRUD controller/service denylist, command routing registry if present, governance test files, report files. Do not touch UOM/item/schema implementation in Wave 0.
4. Ensure governed resources cannot be mutated through Generic CRUD: ITEM, CUST, SUP, EQP, MDEV, UOM, BOM, ROUTING, CONTROL_PLAN, INSPECTION_PLAN, TOOL, GAGE, FIXTURE, EMPLOYEE/USER, LOT/SERIAL, HOLD/NCR/CAPA.
5. Add focused tests proving `409 domain_command_required` or equivalent fail-closed response for governed writes, with problem details and correlation id if available.
6. Run focused syntax/PHPUnit/PHPStan/grep checks; if full suite blocked by existing debt, record exact blocker and focused pass evidence.
7. Commit every logical unit and do not merge to main.
...
```
```bash
git branch -r | sed -n "1,80p"
```
```bash
grep -R "domain_command_required\|GenericCrud" -n mom/api mom/tests docs | head -120
```
```bash
php -l mom/api/controllers/GenericCrudController.php || true
```
```bash
composer --working-dir=mom run test -- --filter GenericCrud || true
```
```bash
...
```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P27_implementation_wave0_authority_freeze_codex_prompt_HANDOFF_PACKET.md
```
...
- Generic CRUD attempts to release supplier approval.
- Generic CRUD attempts to clear quality hold.
- Dashboard uses generic endpoint to mutate machine readiness.
- Ungoverned reference read remains allowed.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- Any governed mutation still open through Generic CRUD is P0/P1.
- Any broad rewrite or framework replacement is P0.
- Any uncommitted changes at handoff are P1 unless explicitly recorded.
