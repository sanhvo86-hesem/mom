# P01 — Current backend authority audit and repo reality map
...
| --- | --- |
| Prompt ID | `P01` |
| Title | Current backend authority audit and repo reality map |
| Mode | `PLANNING_AND_AUDIT_ONLY` |
| Required previous token | `P00_PASS_READY_FOR_NEXT` |
| Unlock token | `P01_PASS_WITH_CONTROLLED_GAPS` |
| Domain affected | `master_data + platform_backend` |
| Root / scope code | `MDA-AUDIT` |

## Prompt objective

Build a precise reality map of current backend authority for master data, JSON/PG runtime modes, generic CRUD mitigation, command coverage, UOM status, engineering definitions, machine/tool/operator resources, and controlled gaps.

## Previous gate check

1. Open `_reports/agent-audits/mda-prompt-os-v2-2026-05-30/MDA_V2_SEQUENCE_STATE.json`.
2. Verify that required previous token `P00_PASS_READY_FOR_NEXT` is present unless this is `P00`.
...
- `mom/database/migrations/080_seed_master_data_from_json.sql`
...
1. Read prior V1 MDA reports if present under `_reports/agent-audits/mda-prompt-os-2026-05-29/`, but verify current repo state independently.
2. Use `.ai/db-map/index.json`, `.ai/symbols.json`, `.ai/contracts-map.json`, and `.ai/route-map.json` before opening source files.
3. Audit `MasterDataService`, `JsonMasterDataRepository`, `DataLayer`, `RuntimeShadowSync`, `GenericCrudController`, command services, relevant migrations, contracts, tests, and runtime assurance tools.
4. Classify each root/collection/table as `JSON_ONLY`, `SHADOW_WRITE`, `POSTGRES_PRIMARY`, `POSTGRES_ONLY`, `SCHEMA_ONLY`, or `NOT_EVIDENCED`.
5. Identify hidden authority risks: metadata JSONB, workspace projection mutation, direct controller file reads, generic CRUD write paths, stale projections, duplicate ID aliases, runtime fallback behavior.
6. Produce a root-by-root authority map for ITEM, CUST, SUP, EQP, MDEV plus UOM, WORK_CENTER, TOOL, GAGE, FIXTURE, EMPLOYEE, USER, BOM, ROUTING, CONTROL_PLAN, INSPECTION_PLAN, WAREHOUSE_LOCATION, LOT, SERIAL, HOLD, NCR, CAPA.
7. Do not change code in P01 unless the prompt is rerun in explicit repair mode after P0/P1 audit defects are found and code mutation is separately authorized by an implementation prompt.
...
```bash
grep -R "class MasterDataService" -n mom/api mom/database docs .ai
```
```bash
grep -R "master_data_store" -n mom docs .ai _reports | head -50
```
```bash
grep -R "mdm_uom_conversions\|CREATE TABLE IF NOT EXISTS uom" -n mom/database docs .ai
```
```bash
php -l mom/api/services/MasterDataService.php || true
```
```bash
php -l mom/database/DataLayer.php || true
```

## Required outputs for this prompt

```text
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_MAIN.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_SOURCE_TRUTH_AUDIT.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_AUTHORITY_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_OPERATIONAL_SIMULATION_MATRIX.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_ADVERSARIAL_REVIEW.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_GAP_AND_REPAIR_LEDGER.csv
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_VERIFICATION_LOG.md
_reports/agent-audits/mda-prompt-os-v2-2026-05-30/P01_current_backend_authority_reality_audit_HANDOFF_PACKET.md
```
...
- SO release tries to use JSON-only customer item approval.
- WO release tries to use schema-only UOM conversion.
- Generic CRUD attempts to update released item status.
- MasterDataController reads missing legacy JSON file.
- RuntimeShadowSync drops BOM/routing/control plan from rebuild.

Also add any new scenarios discovered during audit.

## Prompt-specific stop rules

- False claim of PG authority without live service evidence is P1.
- Any ignored UOM authority gap is P1.
- Any mutation path called safe without command/audit/evidence proof is P1.
